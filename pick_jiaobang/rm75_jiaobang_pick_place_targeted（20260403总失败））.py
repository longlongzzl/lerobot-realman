...
from object_specs import get_object_spec, normalize_object_name, resolve_object_spec_scales
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--tabletop-place-slot-jitter-m",
        type=float,
        nargs="*",
        default=[0.0, 0.02],
        help="For tabletop place_on_slots rules, also try small local x/z offsets around each slot center. This helps when the exact slot center is IK-infeasible but a nearby tabletop pose is reachable.",
    )
    return parser
...
def _get_cached_scene_object_world_transform(demo, bridge_mod, scene_capture_cache, object_name: str | None) -> np.ndarray | None:
    normalized = normalize_object_name(object_name)
    if normalized is None:
        return None
...
def _project_to_world_table_plane(vec: np.ndarray) -> np.ndarray | None:
    world_up = np.array([0.0, 0.0, 1.0], dtype=np.float32)
    vec = np.asarray(vec, dtype=np.float32).reshape(3)
    projected = vec - float(np.dot(vec, world_up)) * world_up
    return _normalize(projected)


def _canonicalize_desk_target_transform(T_world_target: np.ndarray) -> np.ndarray:
    T_world_target = np.asarray(T_world_target, dtype=np.float32).reshape(4, 4).copy()
    world_up = np.array([0.0, 0.0, 1.0], dtype=np.float32)
    x_proj = _project_to_world_table_plane(T_world_target[:3, 0])
    z_proj = _project_to_world_table_plane(T_world_target[:3, 2])
    if x_proj is not None:
        x_axis = x_proj
        z_axis = _normalize(np.cross(x_axis, world_up))
        if z_axis is not None and z_proj is not None and float(np.dot(z_axis, z_proj)) < 0.0:
            x_axis = -x_axis
            z_axis = -z_axis
    elif z_proj is not None:
        z_axis = z_proj
        x_axis = _normalize(np.cross(world_up, z_axis))
        if x_axis is not None:
            x_axis = x_axis.astype(np.float32)
        if x_axis is not None and float(np.dot(np.cross(x_axis, world_up), z_axis)) < 0.0:
            x_axis = -x_axis
            z_axis = -z_axis
    else:
        x_axis = None
    ...
    if x_axis is not None and z_axis is not None:
        T_world_target[:3, :3] = np.column_stack([x_axis, world_up, z_axis]).astype(np.float32)

    desk_spec = get_object_spec("desk")
    if desk_spec is None:
        return T_world_target
    asset_file = str(Path(desk_spec.sim_asset_file or desk_spec.mesh_file).expanduser())
    try:
        _, sim_scale = resolve_object_spec_scales(desk_spec)
        pos, _, _ = base.clamp_asset_pose_above_table(
            T_world_target[:3, 3],
            base.bridge_mod_mat2quat(T_world_target[:3, :3]).astype(np.float32),
            asset_file,
            float(sim_scale),
            min_clearance=0.001,
        )
        T_world_target[:3, 3] = np.asarray(pos, dtype=np.float32).reshape(3)
    except Exception:
        pass
    return T_world_target
...
def _get_scene_object_world_transform(demo, bridge_mod, scene_capture_cache, object_name: str | None) -> np.ndarray | None:
    ...
    if normalized is None:
        return None
    cached_T_world_obj = _get_cached_scene_object_world_transform(demo, bridge_mod, scene_capture_cache, normalized)
    if normalized == "desk":
        raw_T_world_obj = cached_T_world_obj
        if raw_T_world_obj is None:
            scene_entry = _find_scene_object_entry(demo, normalized)
            if scene_entry is not None and scene_entry.get("T_world_obj") is not None:
                raw_T_world_obj = np.asarray(scene_entry["T_world_obj"], dtype=np.float32).reshape(4, 4)
        if raw_T_world_obj is None:
            return None
        return _canonicalize_desk_target_transform(raw_T_world_obj)
    scene_entry = _find_scene_object_entry(demo, normalized)
    if scene_entry is not None and scene_entry.get("T_world_obj") is not None:
...
def _make_tabletop_slot_local_pose_variants(
    args,
    rule: PlaceRule,
    object_pose_local: LocalPoseSpec,
) -> list[tuple[str | None, LocalPoseSpec]]:
    variants: list[tuple[str | None, LocalPoseSpec]] = [(None, object_pose_local)]
    if rule.primitive != "place_on_slots":
        return variants

    jitter_values = [
        abs(float(v))
        for v in list(getattr(args, "tabletop_place_slot_jitter_m", []) or [])
        if np.isfinite(float(v))
    ]
    if not jitter_values:
        return variants

    px, py, pz = [float(v) for v in object_pose_local.position]
    rpy_deg = tuple(float(v) for v in object_pose_local.rpy_deg)
    seen = {(round(px, 6), round(py, 6), round(pz, 6))}
    for jitter_m in jitter_values:
        if jitter_m <= 1e-6:
            continue
        for dx, dz in ((jitter_m, 0.0), (-jitter_m, 0.0), (0.0, jitter_m), (0.0, -jitter_m)):
            pos = (px + float(dx), py, pz + float(dz))
            key = (round(pos[0], 6), round(pos[1], 6), round(pos[2], 6))
            if key in seen:
                continue
            seen.add(key)
            label = f"slot_dx{int(round(dx * 1000.0)):+d}_dz{int(round(dz * 1000.0)):+d}mm"
            variants.append((label, LocalPoseSpec(position=pos, rpy_deg=rpy_deg)))
    return variants
...
def build_targeted_place_plan_variants(demo, bridge_mod, scene_capture_cache, rule: PlaceRule, place_state_cache, args) -> list[TargetedPlacePlan]:
    ...
    for object_pose_local, slot_name in slot_specs:
        slot_local_pose_variants = _make_tabletop_slot_local_pose_variants(args, rule, object_pose_local)
        for slot_variant_label, slot_object_pose_local in slot_local_pose_variants:
            local_variants = _make_insert_vertical_local_pose_variants(
                rule,
                slot_object_pose_local,
                getattr(args, "insert_vertical_axial_spin_deg", None),
            )
            if rule.primitive == "place_on_slots":
                local_variants = _make_orientation_invariant_local_pose_variants(rule, slot_object_pose_local)
                if len(local_variants) <= 1:
                    local_variants = _make_tabletop_axial_spin_local_pose_variants(args, rule, slot_object_pose_local)
            ...
            for local_variant_label, T_target_obj_desired in local_variants:
                ...
                for tabletop_variant_label, T_world_obj_desired in _make_tabletop_place_world_pose_variants(
                    ...
                    labels = [label for label in (slot_variant_label, local_variant_label, tabletop_variant_label) if label]
                    ...
                    pre_place_pose = base.make_pose_with_position(place_pose, p_place + np.array([0.0, 0.0, float(rule.hover_height)], dtype=np.float32))
                    staging_pose = None
...
def _run_post_grasp_escape_for_place(demo, bridge_mod, real_exec, args, label: str, *, use_attach: bool) -> bool:
    ...
    if lift_height <= 1e-6:
        return True
    escape_candidates = [
        (lift_height, retreat_distance),
        (max(lift_height + 0.03, lift_height * 1.4), max(retreat_distance, 0.03)),
        (max(lift_height + 0.06, lift_height * 1.8, 0.10), max(retreat_distance, 0.05)),
    ]
    tried = set()
    deduped_candidates = []
    ...
    for attempt_idx, (candidate_lift, candidate_retreat) in enumerate(deduped_candidates, start=1):
        ...
        print(f"\n[{candidate_label}]")
        lift_pose = base.make_lifted_tcp_pose(demo, candidate_lift, retreat_distance=candidate_retreat)
...
def run_targeted_place_episode(demo, bridge_mod, real_exec, args, scene_capture_cache, place_state_cache) -> bool:
    ...
    if not ok:
        ...
        if not ok:
            ok, _ = base.retry_pregrasp_via_sim_start(
                demo,
                bridge_mod,
                real_exec,
                "pregrasp",
    ...
    for candidate in place_plan_candidates:
        ...
        if q_pre_place_path is not None:
            ...
            try:
                last_place_pose = candidate.place_pose
                q_place_candidate = base.plan_pose_path(
                    demo,
                    candidate.place_pose,
                    variant_name=args.variant,
                    use_attach=True,
