...
def register_planner_virtual_top_wall(env, demo, args):
    ...
    wall_z = float(getattr(args, "planner_virtual_top_wall_z", 0.80))
...
def build_arg_parser():
    ...
    parser.add_argument("--pen-topdown-grasp-yaw-variant-deg", type=float, nargs="*", default=[0.0, 180.0], help="For pen_topdown_insert_ready grasp mode, only try this narrower set of world-z yaw variants so the pen stays in insertion-friendly wrist branches.")
    parser.add_argument("--pen-topdown-tilt-toward-robot-deg", type=float, nargs="*", default=[0.0, 12.0], help="For pen_topdown_insert_ready grasp mode, also try these small tilt angles toward the robot when direct top-down pregrasp/grasp IK fails.")
    parser.add_argument("--pen-topdown-tilt-toward-robot-shift-m", type=float, nargs="*", default=[0.0, 0.01], help="For pen_topdown_insert_ready grasp mode, also try these small shifts toward the robot for tilted candidates.")
    ...
    parser.add_argument("--pen-grasp-fallback-z-lifts", type=float, nargs="*", default=[0.008, 0.014], help="Extra world-z lifts to try for pen_topdown_insert_ready grasp fallback candidates.")
    parser.add_argument("--pen-grasp-fallback-approach-backoffs", type=float, nargs="*", default=[0.010, 0.018], help="Extra approach-axis backoff distances to try for pen_topdown_insert_ready grasp fallback candidates.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Reject planned paths whose TCP cumulative rotation or excursion from the start orientation exceeds this many full turns. Set <=0 to disable.",
    )
    parser.add_argument(
        "--rrt-attempt-count",
        type=int,
        default=3,
        help="How many increasingly aggressive RRT parameter settings to try for each pose/joint planning request before giving up. When multiple attempts succeed, the planner keeps the shortest successful path.",
    )
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="When multiple RRT attempts succeed, keep planning long enough to compare them and execute the shortest successful joint path instead of the first one that succeeds. Enabled by default.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="RRTConnect",
        help="Underlying OMPL planner name used by mplib for pose/joint planning. Supported by this mplib build: RRTConnect, RRTstar. Defaults to RRTConnect.",
    )
    parser.add_argument(
        "--relaxed-pose-ik-fallback",
        dest="relaxed_pose_ik_fallback",
        action="store_true",
        default=True,
        help="When pose planning fails only because the IK residual is slightly above the strict default threshold, retry selected non-grasp motions with a relaxed IK acceptance threshold. Enabled by default.",
    )
    parser.add_argument(
        "--no-relaxed-pose-ik-fallback",
        dest="relaxed_pose_ik_fallback",
        action="store_false",
        help="Disable the relaxed IK fallback for selected pose-planning stages.",
    )
    parser.add_argument(
        "--relaxed-pose-ik-distance-threshold",
        type=float,
        default=0.01,
        help="Relaxed IK distance threshold in meters for selected non-grasp pose-planning fallbacks such as place/staging/escape motions.",
    )
...
def _get_tcp_max_turns(demo) -> float:
    demo_args = getattr(demo, "args", None)
    if demo_args is None:
        return 1.0
    return float(max(getattr(demo_args, "tcp_max_turns", 1.0), 0.0))


def _get_planner_name(demo) -> str:
    demo_args = getattr(demo, "args", None)
    raw = "RRTConnect" if demo_args is None else str(getattr(demo_args, "planner_name", "RRTConnect") or "RRTConnect")
    name = raw.strip()
    if not name:
        return "RRTConnect"
    name_l = name.lower().replace(" ", "")
    if name_l in {"rrt*", "rrtstar", "rrt_star"}:
        return "RRTstar"
    if name_l in {"rrtconnect", "rrt_connect"}:
        return "RRTConnect"
    return "RRTConnect"


def _rotation_angle_between_quats_wxyz(q0, q1) -> float:
    R0 = quat2mat(_normalize_quat_wxyz(q0)).astype(np.float64)
    R1 = quat2mat(_normalize_quat_wxyz(q1)).astype(np.float64)
    R_rel = R1 @ R0.T
...
def _collect_tcp_quats_along_joint_path(demo, q_start: np.ndarray, q_path) -> list[np.ndarray]:
    q_saved = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    tcp_quats: list[np.ndarray] = []
    try:
        sync_demo_arm_qpos(demo, q_start)
        tcp_quats.append(_normalize_quat_wxyz(flatten_np(demo.tcp.pose.q)[:4]).astype(np.float32))
...
def _tcp_rotation_metrics(demo, q_start: np.ndarray, q_path) -> dict:
    tcp_quats = _collect_tcp_quats_along_joint_path(demo, q_start, q_path)
    if len(tcp_quats) <= 1:
        return {
            "tcp_total_rotation": 0.0,
            "tcp_max_excursion": 0.0,
        }
    ...
    total_rotation = 0.0
    max_excursion = 0.0
    for q_curr in tcp_quats[1:]:
        total_rotation += _rotation_angle_between_quats_wxyz(q_prev, q_curr)
        max_excursion = max(max_excursion, _rotation_angle_between_quats_wxyz(q_ref, q_curr))
        q_prev = q_curr
    return {
        "tcp_total_rotation": float(total_rotation),
        "tcp_max_excursion": float(max_excursion),
    }
...
def validate_tcp_turn_budget(demo, q_start: np.ndarray, q_path, *, label: str) -> bool:
    max_turns = _get_tcp_max_turns(demo)
    if max_turns <= 0.0:
        return True
    limit = float(max_turns * 2.0 * np.pi)
    metrics = _tcp_rotation_metrics(demo, q_start, q_path)
    total_rotation = float(metrics["tcp_total_rotation"])
    max_excursion = float(metrics["tcp_max_excursion"])
    tol = 1e-3
    if total_rotation <= limit + tol and max_excursion <= limit + tol:
        return True
    print(
        f"[planner] {label} rejected by TCP turn budget: "
        f"tcp_total_rotation={total_rotation:.3f} rad, tcp_max_excursion={max_excursion:.3f} rad, "
        f"limit={limit:.3f} rad ({max_turns:.2f} turns)"
    )
    return False


def _retarget_single_joint_target_with_turn_budgets(
    demo,
    q_start: np.ndarray,
    q_target: np.ndarray,
    *,
    ...
    q_adjusted = retarget_goal_q_to_nearest_equivalent(demo, q_target, q_start)
    q_adjusted = _retarget_joint7_with_turn_budget(demo, q_adjusted, q_start, q_start)
    if q_adjusted is None:
        print(f"[planner] {label} rejected: joint7 could not be retargeted within the configured turn budget")
        return None
    q_single_path = [np.asarray(q_adjusted, dtype=np.float32).reshape(-1)[:7]]
    if not validate_joint7_turn_budget(demo, q_start, q_single_path, label=label):
        return None
    if not validate_tcp_turn_budget(demo, q_start, q_single_path, label=label):
        return None
    return np.asarray(q_adjusted, dtype=np.float32).reshape(-1)[:7]
...
def _joint_path_quality_metrics(demo, q_start: np.ndarray, q_path) -> dict:
    q_start = np.asarray(q_start, dtype=np.float32).reshape(-1)[:7]
    ...
    tcp_metrics = _tcp_rotation_metrics(demo, q_start, q_path)
    return {
        ...
        "tcp_total_rotation": float(tcp_metrics["tcp_total_rotation"]),
        "tcp_max_excursion": float(tcp_metrics["tcp_max_excursion"]),
    }


def _joint_path_quality_score(metrics: dict) -> float:
    return float(metrics["total_motion"])


def _select_best_rrt_success_candidate(demo, label: str, q_start: np.ndarray, candidates):
    if not candidates:
        return None
    if len(candidates) == 1:
        candidate = dict(candidates[0])
        candidate["metrics"] = _joint_path_quality_metrics(demo, q_start, candidate["path"])
    ...
    for candidate in candidates:
        ...
        metrics = _joint_path_quality_metrics(demo, q_start, candidate["path"])
        ...
        print(
            ...
            f"tcp_total={metrics['tcp_total_rotation']:.3f} rad, "
            f"tcp_excursion={metrics['tcp_max_excursion']:.3f} rad, "
    ...
    best = min(
        ...
        key=lambda item: (
            ...
            float(item["metrics"]["tcp_total_rotation"]),
    ...
    print(
        ...
        f"tcp_total={best['metrics']['tcp_total_rotation']:.3f} rad, "
        f"tcp_excursion={best['metrics']['tcp_max_excursion']:.3f} rad, "
    ...
    def _run_rrt_attempts(reverse: bool = False):
        ...
        for attempt_name, attempt_time, attempt_range in attempt_configs:
            ...
            result = demo.planner.plan_qpos_to_qpos(
                ...
                use_attach=use_attach,
                planner_name=_get_planner_name(demo),
            )
            ...
            if not validate_joint7_turn_budget(demo, q_start, q_path, label=path_label):
                continue
            if not validate_tcp_turn_budget(demo, q_start, q_path, label=path_label):
                continue
        ...
        if success_candidates:
            best_candidate = _select_best_rrt_success_candidate(demo, path_label, q_start, success_candidates)
...
def _should_try_relaxed_pose_ik_fallback(demo, label: str, status) -> bool:
    demo_args = getattr(demo, "args", None)
    if demo_args is None or not bool(getattr(demo_args, "relaxed_pose_ik_fallback", True)):
        return False
    relaxed_threshold = float(getattr(demo_args, "relaxed_pose_ik_distance_threshold", 0.0) or 0.0)
    if relaxed_threshold <= 1e-3:
        return False
    status_str = str(status or "")
    if not status_str.startswith("IK Failed! Distance "):
        return False
    label_l = str(label or "").lower()
    if any(token in label_l for token in ("place", "staging", "escape", "sim_start", "fixed_goal")):
        return True
    return False


def _plan_qpos_to_pose_with_relaxed_ik(
    demo,
    target,
    q_start: np.ndarray,
    *,
    ...
    demo_args = getattr(demo, "args", None)
    relaxed_threshold = float(getattr(demo_args, "relaxed_pose_ik_distance_threshold", 0.01) or 0.01)
    print(
        f"[planner] {label} retrying pose IK with relaxed threshold={relaxed_threshold:.4f} m"
    )
    ik_status, goal_qpos = demo.planner.IK(target, q_start.copy(), threshold=relaxed_threshold)
    print(f"[planner] {label} relaxed IK status:", ik_status)
    if ik_status != "Success" or not goal_qpos:
    ...
    print(f"[planner] {label} relaxed-IK qpos plan status:", result.get("status", result))
    return result
...
def plan_pose_path(demo, pose, *, variant_name: str = "array_wxyz", use_attach: bool = False, label: str = "pose_goal", planning_time: float = 4.0, rrt_range: float = 0.15, start_q=None, allow_start_in_collision: bool = False):
    ...
    for attempt_name, attempt_time, attempt_range in attempt_configs:
        ...
        if result.get("status", "") != "Success" and _should_try_relaxed_pose_ik_fallback(demo, label, last_status):
            relaxed_result = _plan_qpos_to_pose_with_relaxed_ik(
                demo,
                target,
                q_start,
                use_attach=use_attach,
            ...
            relaxed_status = relaxed_result.get("status", relaxed_result)
            if relaxed_result.get("status", "") == "Success":
                result = relaxed_result
                last_status = relaxed_status
            else:
                last_status = relaxed_status
        if result.get("status", "") != "Success":
            continue
        ...
        if not validate_joint7_turn_budget(demo, q_start, q_path, label=label):
            continue
        if not validate_tcp_turn_budget(demo, q_start, q_path, label=label):
            continue
        if not validate_joint_path_segments(
            demo,
            q_start,
    ...
    if success_candidates:
        best_candidate = _select_best_rrt_success_candidate(demo, label, q_start, success_candidates)
...
def retry_pregrasp_via_sim_start(
    demo,
    bridge_mod,
    real_exec: RealmanJointExecutor | None,
    label: str,
    ...
    sim_start_q = np.asarray(getattr(demo, "_cycle_start_arm_q", demo.current_arm_qpos()), dtype=np.float32).reshape(-1)[:7]
    q_current = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    if np.allclose(q_current, sim_start_q, atol=1e-3):
        print(f"[planner] {label} sim_start fallback skipped because the arm is already near this cycle's start configuration")
        return False, None

    print(f"[planner] {label} pre-execution escape retry failed; trying a final fallback via this cycle's sim_start arm configuration")
    reset_label = f"{label}_via_sim_start"

    def _plan_reset_path(q_start_local: np.ndarray):
        return plan_joint_path(
            demo,
            sim_start_q,
            use_attach=False,
            ...
            planning_time=4.0,
            rrt_range=0.15,
            start_q=q_start_local,
            allow_linear_fallback=True,
            allow_reverse_rrt_fallback=True,
            prefer_reverse_rrt=True,
        )

    q_reset_path = _plan_reset_path(q_current)
    if q_reset_path is None:
        base_retreat = float(max(getattr(args, "pregrasp_blocked_retry_retreat_distance", 0.0), 0.0))
        base_lift = float(max(getattr(args, "pregrasp_blocked_retry_lift_height", 0.0), 0.0))
        pre_escape_candidates: list[tuple[float, float]] = []
        for retreat_m, lift_m in [
        ...
        q_reset_start = q_current.copy()
        for candidate_idx, (candidate_retreat_m, candidate_lift_m) in enumerate(pre_escape_candidates, start=1):
            print(
                f"[planner] {reset_label} planning failed; trying a local obstacle-escape candidate "
                f"{candidate_idx}/{len(pre_escape_candidates)} "
                f"(retreat={candidate_retreat_m:.3f} m, lift={candidate_lift_m:.3f} m)"
            )
            escape_label = (
                f"{reset_label}_pre_escape"
                if len(pre_escape_candidates) == 1
                else f"{reset_label}_pre_escape_r{int(round(candidate_retreat_m * 1000.0))}_z{int(round(candidate_lift_m * 1000.0))}"
            )
            escape_pose = make_lifted_tcp_pose(
                demo,
                candidate_lift_m,
            ...
            ok_escape, q_escape_sent = execute_pose_path_stage(
                demo,
                bridge_mod,
                real_exec,
                escape_label,
            ...
            q_reset_start = np.asarray(q_escape_sent, dtype=np.float32).reshape(-1)[:7]
            q_reset_path = _plan_reset_path(q_reset_start)
            if q_reset_path is not None:
                break

    if q_reset_path is None:
        print(f"[warn] {label}_via_sim_start planning failed")
        return False, None

    ok_reset, q_sent = execute_joint_path_stage(
        demo,
        bridge_mod,
        real_exec,
        reset_label,
    ...
    if not ok_reset:
        print(f"[warn] {label}_via_sim_start execution failed")
        return False, None

    q_retry_path = plan_pose_path(
        demo,
        pregrasp_pose,
        variant_name=args.variant,
        use_attach=False,
        label=f"{label}_after_sim_start",
        start_q=np.asarray(q_sent, dtype=np.float32).reshape(-1)[:7],
    )
    if q_retry_path is not None:
        ok_retry, q_retry_sent = execute_pose_path_stage(
            demo,
            bridge_mod,
            real_exec,
            f"{label}_after_sim_start",
            pregrasp_pose,
            q_retry_path,
            gripper_pos,
            args,
        ...
        if ok_retry:
            return True, q_retry_sent

    print(f"[planner] {label}_after_sim_start pose-path planning failed; falling back to terminal joint target")
    q_retry = demo.plan_terminal_q(pregrasp_pose, variant_name=args.variant)
    if q_retry is None:
        print(f"[warn] {label}_after_sim_start terminal IK failed")
        return False, None
...
def enforce_topdown_grasp_insertion_limit(demo, args, grasp_pose, pregrasp_pose):
    ...
    if grasp_mode not in {"topdown_long_axis", "pen_topdown_insert_ready"}:
        return grasp_pose, pregrasp_pose, 0.0
...
def build_grasp_pose_variants(demo, grasp_pose, args):
    ...
    if grasp_mode == "pen_topdown_insert_ready":
        tilt_degs = [float(v) for v in getattr(args, "pen_topdown_tilt_toward_robot_deg", []) or [] if abs(float(v)) > 1e-6]
        shift_ds = [float(v) for v in getattr(args, "pen_topdown_tilt_toward_robot_shift_m", []) or [] if float(v) >= 0.0]
        yaw_degs = [float(v) for v in getattr(args, "pen_topdown_grasp_yaw_variant_deg", []) or []]
    else:
...
def plan_grasp_pose_with_fallbacks(demo, bridge_mod, grasp_pose, args, *, base_label: str = "grasp"):
    ...
    grasp_mode = str(getattr(args, "grasp_mode", "object_normal") or "object_normal").strip().lower()
    if grasp_mode == "pen_topdown_insert_ready":
        z_lifts = [float(v) for v in getattr(args, "pen_grasp_fallback_z_lifts", []) or [] if float(v) > 1e-6]
        backoffs = [float(v) for v in getattr(args, "pen_grasp_fallback_approach_backoffs", []) or [] if float(v) > 1e-6]
    else:
...
def execute_stage(demo, bridge_mod, real_exec: RealmanJointExecutor | None, label: str, pose, q_target: np.ndarray, gripper_pos: float, args):
    q_target = np.asarray(q_target, dtype=np.float32).reshape(-1)[:7]
    q_current = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    q_target = _retarget_single_joint_target_with_turn_budgets(demo, q_current, q_target, label=label)
    if q_target is None:
        return False, None
...
def run_real_episode(demo, bridge_mod, real_exec: RealmanJointExecutor | None, args) -> bool:
    ...
    demo._cycle_start_arm_q = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7].copy()
    ...
    for grasp_variant_label, grasp_variant_pose in grasp_pose_variants:
        ...
        if q_pre_path is not None:
            ...
            if not ok:
                ...
                if not ok:
                    ok, _ = retry_pregrasp_via_sim_start(
                        demo,
                        bridge_mod,
                        real_exec,
                        "pregrasp" if grasp_variant_label == "grasp" else f"pregrasp_{grasp_variant_label}",
    ...
    def _run_post_grasp_escape(lift_height: float, label: str, retreat_distance: float = 0.0) -> bool:
        if float(lift_height) <= 1e-6:
            return True
        escape_candidates = [
            (float(lift_height), float(retreat_distance)),
            (max(float(lift_height) + 0.03, float(lift_height) * 1.4), max(float(retreat_distance), 0.03)),
            (max(float(lift_height) + 0.06, float(lift_height) * 1.8, 0.10), max(float(retreat_distance), 0.05)),
        ]
        tried = set()
        deduped_candidates = []
        for candidate_lift, candidate_retreat in escape_candidates:
            key = (round(float(candidate_lift), 4), round(float(candidate_retreat), 4))
            if key in tried:
                continue
            tried.add(key)
            deduped_candidates.append((float(candidate_lift), float(candidate_retreat)))

        for attempt_idx, (candidate_lift, candidate_retreat) in enumerate(deduped_candidates, start=1):
            candidate_label = label if attempt_idx == 1 else f"{label}_retry_h{int(round(candidate_lift * 1000.0))}"
            if attempt_idx > 1:
                print(
                    f"[planner] {label} retrying with a higher escape lift "
                    f"(candidate {attempt_idx}/{len(deduped_candidates)}: lift={candidate_lift:.3f} m, retreat={candidate_retreat:.3f} m)"
                )
            print(f"\n[{candidate_label}]")
            lift_pose = make_lifted_tcp_pose(demo, candidate_lift, retreat_distance=candidate_retreat)
            ...
            if lift_path is None:
                print(f"[warn] {candidate_label} planning failed")
                continue
            ok, _ = execute_pose_path_stage(
                demo,
                bridge_mod,
            ...
            if not ok:
                print(f"[warn] {candidate_label} execution failed")
                continue
            lift_active_object_above_table_if_needed(
                demo,
                args,
