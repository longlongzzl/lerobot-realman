...
from place_rules import get_runtime_slot_specs


DEFAULT_BRIDGE_SCRIPT = "/home/zhangzhao/Desktop/lerobot/pick_jiaobang/rm75_jiaobang_pick_move_with_foundationpose.py"
DEFAULT_PICK_SCRIPT = "/home/zhangzhao/Desktop/lerobot/pick_jiaobang/rm75_jiaobang_pick_move_v10_perpendicular_to_object.py"
DEFAULT_LEROBOT_ROOT = "/home/zhangzhao/Desktop/lerobot"
DEFAULT_LEROBOT_SIM2REAL_ROOT = "/home/zhangzhao/Desktop/lerobot/lerobot-sim2real"
...
def register_scene_obstacles(env, demo, bridge_mod, T_base_cam: np.ndarray, scene_obstacles, args):
    ...
    for item in scene_obstacles:
        ...
        box_scale = float(global_box_scale * object_box_scale)
        ...
        print(
            ...
            f"planner_collision={'yes' if planner_collision else 'no'}, "
            f"box_scale={box_scale:.3f} (global={global_box_scale:.3f}, object={object_box_scale:.3f}), "
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--debug-dir",
        type=str,
        default="/home/zhangzhao/Desktop/lerobot/debug_jiaobang_foundationpose_real",
    )
    ...
    parser.add_argument(
        ...
        dest="lock_object_z_to_table",
        action="store_true",
        default=True,
        help="During FoundationPose scene mapping/refinement, force mapped tabletop objects to sit on the table by aligning their lowest point to the table plane. Enabled by default.",
    )
    parser.add_argument(
        "--no-lock-object-z-to-table",
        dest="lock_object_z_to_table",
        action="store_false",
        help="Disable the tabletop-contact lock during FoundationPose mapping/refinement and only prevent objects from penetrating the table.",
    )
    ...
    parser.add_argument(
        "--pregrasp-blocked-retry-max-segment-joint-delta",
        type=float,
        default=0.60,
        help="Per-joint delta budget used when validating local IK segments for pregrasp pre-escape motions. Larger values make local obstacle-escape more permissive.",
    )
    parser.add_argument(
        "--pregrasp-blocked-retry-max-segment-joint7-delta",
        type=float,
        default=1.20,
        help="Joint7 delta budget used when validating local IK segments for pregrasp pre-escape motions.",
    )
    parser.add_argument(
        "--pregrasp-blocked-retry-max-segment-norm-delta",
        type=float,
        default=1.00,
        help="Joint-space norm delta budget used when validating local IK segments for pregrasp pre-escape motions.",
    )
    ...
    parser.add_argument("--grasp-fallback-z-lifts", type=float, nargs="*", default=[0.012, 0.02, 0.028], help="Extra world-z lifts to try when direct grasp IK fails.")
    ...
    parser.add_argument(
        "--grasp-local-descent",
        dest="grasp_local_descent",
        action="store_true",
        default=True,
        help="When descending from pregrasp/approach to the final grasp, prefer a local Cartesian descent instead of a global pose-RRT. Enabled by default.",
    )
    parser.add_argument(
        "--no-grasp-local-descent",
        dest="grasp_local_descent",
        action="store_false",
        help="Disable the local Cartesian grasp-descent preference and use the previous global pose planning behavior.",
    )
    parser.add_argument(
        "--grasp-local-descent-allow-pose-rrt-fallback",
        dest="grasp_local_descent_allow_pose_rrt_fallback",
        action="store_true",
        default=False,
        help="If the local Cartesian grasp descent fails, fall back to a global pose-RRT to reach the grasp pose. Disabled by default because it may rotate while descending.",
    )
    ...
    parser.add_argument(
        "--segmented-pose-rrt",
        dest="segmented_pose_rrt",
        action="store_true",
        default=False,
        help="For long pose-goal motions, insert task-space straight-line intermediate poses and solve them one segment at a time before falling back to one-shot global pose RRT. Disabled by default.",
    )
    parser.add_argument(
        "--no-segmented-pose-rrt",
        dest="segmented_pose_rrt",
        action="store_false",
        help="Disable segmented task-space pose planning and use one-shot global pose RRT directly.",
    )
    parser.add_argument(
        "--segmented-pose-max-translation",
        type=float,
        default=0.18,
        help="Maximum task-space translation per segmented pose-RRT subgoal in meters.",
    )
    parser.add_argument(
        "--segmented-pose-max-rotation-deg",
        type=float,
        default=35.0,
        help="Maximum task-space rotation per segmented pose-RRT subgoal in degrees.",
    )
    parser.add_argument(
        "--segmented-pose-rrt-attempt-count",
        type=int,
        default=3,
        help="RRT attempt budget for each segmented pose-RRT subgoal.",
    )
    parser.add_argument(
        "--segmented-pose-rrt-planning-time-scale",
        type=float,
        default=0.75,
        help="Scale factor applied to the base planning_time for each segmented pose-RRT subgoal.",
    )
    ...
    parser.add_argument(
        "--rrt-max-detour-ratio",
        type=float,
        default=1.8,
        help="Reject a planned joint path if its total joint motion exceeds this multiple of the straight-line joint distance from start to goal. Set <=0 to disable.",
    )
    parser.add_argument(
        "--rrt-max-detour-extra",
        type=float,
        default=0.8,
        help="Reject a planned joint path if its total joint motion exceeds the straight-line joint distance by more than this many radians. Set <=0 to disable.",
    )
    parser.add_argument(
        "--adaptive-segmented-joint-planning",
        dest="adaptive_segmented_joint_planning",
        action="store_true",
        default=False,
        help="For joint-goal motions, try an adaptive linear-first local-repair planner before falling back to one-shot global RRT. Disabled by default.",
    )
    parser.add_argument(
        "--no-adaptive-segmented-joint-planning",
        dest="adaptive_segmented_joint_planning",
        action="store_false",
        help="Disable adaptive failed-segment splitting for joint-goal motions and use the previous one-shot global RRT behavior directly.",
    )
    parser.add_argument(
        "--adaptive-segmented-joint-max-depth",
        type=int,
        default=3,
        help="Maximum recursion depth for adaptive failed-segment splitting in joint-goal planning.",
    )
    parser.add_argument(
        "--adaptive-segmented-joint-leaf-max-delta",
        type=float,
        default=0.25,
        help="When a failed joint segment becomes smaller than this max per-joint delta in radians, stop splitting and use a local RRT on that leaf segment.",
    )
    parser.add_argument(
        "--adaptive-segmented-joint-leaf-attempt-count",
        type=int,
        default=2,
        help="How many RRT attempt presets to use on each adaptive leaf segment before giving up.",
    )
    parser.add_argument(
        "--adaptive-segmented-joint-leaf-planning-time-scale",
        type=float,
        default=0.5,
        help="Scale factor applied to planning_time for adaptive leaf-segment local RRTs.",
    )
    parser.add_argument(
        "--pose-ik-candidate-top-k",
        type=int,
        default=4,
        help="After solving IK for a pose goal, keep at most this many high-quality joint candidates after multi-factor ranking.",
    )
    parser.add_argument(
        "--pose-ik-score-joint-distance-weight",
        type=float,
        default=1.0,
        help="Weight for overall joint distance from the current configuration when ranking pose-goal IK candidates.",
    )
    parser.add_argument(
        "--pose-ik-score-joint7-weight",
        type=float,
        default=0.20,
        help="Extra weight on joint7 change when ranking pose-goal IK candidates.",
    )
    parser.add_argument(
        "--pose-ik-score-wrist-weight",
        type=float,
        default=0.12,
        help="Extra weight on wrist-joint motion (joints 4-7) when ranking pose-goal IK candidates.",
    )
    parser.add_argument(
        "--pose-ik-score-limit-weight",
        type=float,
        default=0.35,
        help="Penalty weight for IK candidates that sit close to joint limits.",
    )
    parser.add_argument(
        "--pose-ik-score-template-weight",
        type=float,
        default=0.08,
        help="Penalty weight for deviating from the preferred phase-specific joint template or recent success family.",
    )
    ...
    parser.add_argument(
        "--show-place-slot-markers",
        dest="show_place_slot_markers",
        action="store_true",
        default=True,
        help="Render visual slot markers for place_on_slots targets such as the desk. Enabled by default.",
    )
    parser.add_argument(
        "--no-show-place-slot-markers",
        dest="show_place_slot_markers",
        action="store_false",
        help="Disable visual slot markers for place_on_slots targets.",
    )
    parser.add_argument(
        "--place-slot-marker-radius",
        type=float,
        default=0.008,
        help="Radius of the visual slot marker spheres in meters.",
    )
    parser.add_argument(
        "--place-slot-marker-height-offset",
        type=float,
        default=0.008,
        help="Offset above the target surface used when rendering place slot markers, in the target's local +Y direction.",
    )
    parser.add_argument(
        "--release-settle-steps",
        type=int,
        default=60,
        help="After opening the gripper at a place/fixed goal, keep the arm still and simulate this many extra open-gripper steps so the released object can detach and settle before any retreat/caching.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--pre-transport-lift-height", type=float, default=0.06, help="Before transporting to the fixed joint goal, first lift the grasped object by this many meters in world z. Set to 0 to disable.")
    ...
    parser.add_argument("--transport-lift-height", type=float, default=0.12, help="If direct post-grasp transport to the fixed goal fails, first plan a lift of this many meters in world z before retrying the transport.")
    parser.add_argument(
...
class RealmanJointExecutor:
    ...
    def reset_robot(self, gripper_pos: float | None = None):
        ...
        arm.rm_movej([90, 0, 0, -90, 0, -90, 60], 100, 0, 0, 1)
        try:
...
def _get_scene_obstacle_entry(demo, object_name: str):
...
def register_place_slot_markers(demo, rule, args):
    if not bool(getattr(args, "show_place_slot_markers", True)):
        return []
    if rule is None or str(getattr(rule, "primitive", "") or "") != "place_on_slots":
        return []
    slots = list(getattr(rule, "slots", ()) or [])
    if not slots:
        return []

    target_entry = _get_scene_obstacle_entry(demo, getattr(rule, "target_object_name", None))
    if target_entry is None:
        print(f"[place slot markers] target {getattr(rule, 'target_object_name', None)} not found in the current scene")
        return []

    T_world_target = np.asarray(target_entry.get("T_world_obj"), dtype=np.float32).reshape(4, 4)
    slots = get_runtime_slot_specs(
        getattr(rule, "target_object_name", None),
        slots,
        T_world_target,
        _get_robot_base_world_position(demo),
    )
    asset_file = target_entry.get("asset_file")
    asset_scale = target_entry.get("asset_scale")
    visual_box_size = target_entry.get("visual_box_size")

    local_top_y = None
    try:
        if asset_file is not None and asset_scale is not None:
            local_points = get_asset_local_points(str(asset_file), float(asset_scale))
            local_top_y = float(np.max(local_points[:, 1]))
    except Exception as exc:
        print(f"[place slot markers] failed to infer target top surface from asset geometry: {exc}")

    if local_top_y is None and visual_box_size is not None:
        local_top_y = float(np.asarray(visual_box_size, dtype=np.float32).reshape(3)[1] * 0.5)
    if local_top_y is None:
        local_top_y = 0.0

    radius = float(max(getattr(args, "place_slot_marker_radius", 0.008), 1e-3))
    local_y = float(local_top_y + max(getattr(args, "place_slot_marker_height_offset", 0.008), 0.0))
    colors = [
        (1.0, 0.25, 0.25, 0.95),
        (1.0, 0.55, 0.20, 0.95),
        (0.98, 0.85, 0.20, 0.95),
    ...
    for idx, slot in enumerate(slots, start=1):
        local_slot = np.asarray(slot.object_pose_local.position, dtype=np.float32).reshape(3)
        local_p = np.array([float(local_slot[0]), local_y, float(local_slot[2])], dtype=np.float32)
        world_p = (T_world_target[:3, :3] @ local_p.reshape(3, 1)).reshape(3) + T_world_target[:3, 3]
        actor_name = f"place_slot_marker_{normalize_object_name(rule.target_object_name)}_{slot.name}"
        ...
        actor.set_pose(Pose.create_from_pq(p=world_p.astype(np.float32), q=np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)))
        actors.append(actor)
        print(f"[place slot markers] {slot.name}: world p={np.round(world_p, 6).tolist()}")

    demo.place_slot_marker_actors = actors
    return actors
...
def align_box_pose_to_table_contact(position, quat, box_size, *, min_clearance: float = 0.001):
    pos = np.asarray(position, dtype=np.float32).reshape(3).copy()
    quat = np.asarray(quat, dtype=np.float32).reshape(4).copy()
    size = np.asarray(box_size, dtype=np.float32).reshape(3)
    min_clearance = float(max(min_clearance, 0.0))
...
def align_asset_pose_to_table_contact(position, quat, asset_file: str, asset_scale: float, *, min_clearance: float = 0.001):
    pos = np.asarray(position, dtype=np.float32).reshape(3).copy()
    quat = np.asarray(quat, dtype=np.float32).reshape(4).copy()
    min_clearance = float(max(min_clearance, 0.0))
    try:
        ...
        size = get_asset_box_size(asset_file, asset_scale)
        return align_box_pose_to_table_contact(pos, quat, size, min_clearance=min_clearance)
    world_points = transform_local_points_to_world(pos, quat, local_points)
    bottom_z = float(np.min(world_points[:, 2]))
    delta_z = float(min_clearance - bottom_z)
    pos[2] += delta_z
...
def lock_active_object_z_to_table_if_needed(demo, args, *, min_clearance: float = 0.001) -> float:
    if not bool(getattr(args, "lock_object_z_to_table", True)):
        return 0.0
    obj_pose = demo.base_env.obj.pose
    obj_p = flatten_np(obj_pose.p)[:3].copy()
    obj_q = flatten_np(obj_pose.q)[:4].copy()
    obj_p, delta_z, object_bottom_z = align_asset_pose_to_table_contact(
        obj_p,
        obj_q,
        args.sim_asset_file,
        args.sim_asset_scale,
    ...
    update_attached_box_visual(demo)
    print(
        f"[safety] aligned active object to table contact by {delta_z:+.4f} m "
        f"(bottom z {object_bottom_z:.4f} -> {min_clearance:.4f})"
    )
    return delta_z


def _should_lock_scene_obstacle_to_table(item) -> bool:
    normalized = normalize_object_name(item.get("object_name"))
    actor_name = str(item.get("actor_name", "") or "")
    if bool(item.get("placed", False)):
        return False
    if normalized == "desk":
        return False
    if "virtual_" in actor_name:
        return False
    if normalized is not None and normalized.startswith("virtual_"):
        return False
    return True
...
def lock_scene_obstacles_z_to_table_if_needed(env, demo, args, *, min_clearance: float = 0.001) -> int:
    if not bool(getattr(args, "lock_object_z_to_table", True)):
        return 0
    scene_obstacles = list(getattr(demo, "scene_obstacles", []) or [])
    if not scene_obstacles:
    ...
    aligned_count = 0
    for item in scene_obstacles:
        if not _should_lock_scene_obstacle_to_table(item):
            continue
        actor_name = str(item.get("actor_name", "") or "")
        actor = actors_by_name.get(actor_name)
        asset_file = item.get("asset_file")
        ...
        quat = flatten_np(pose.q)[:4].copy()
        if asset_file is not None and asset_scale is not None:
            pos, delta_z, bottom_z = align_asset_pose_to_table_contact(
                pos,
                quat,
                str(asset_file),
                float(asset_scale),
            ...
            pos, delta_z, bottom_z = align_box_pose_to_table_contact(
                pos,
                quat,
                box_size,
                min_clearance=min_clearance,
        ...
        T_world_obj[:3, 3] = pos.astype(np.float32)
        item["T_world_obj"] = T_world_obj
        item["bottom_z_before_table_lock"] = float(bottom_z)
        item["table_lock_delta_z"] = float(delta_z)
        try:
            if bool(item.get("planner_collision", False)):
                from mplib import collision_detection as mplib_cd
            ...
            print(f"[scene obstacle] {actor_name}: failed to refresh planner collision after table-contact lock: {exc}")
        aligned_count += 1
        print(
            f"[safety] aligned scene obstacle {item.get('object_name', actor_name)} to table contact by {delta_z:+.4f} m "
            f"(bottom z {bottom_z:.4f} -> {float(max(min_clearance, 0.0)):.4f})"
        )
...
def settle_released_object_after_open(demo, args, *, label: str = "release_settle", steps: int | None = None):
    total_steps = int(getattr(args, "release_settle_steps", 60) if steps is None else steps)
    if total_steps <= 0:
        return
    try:
        demo.hold_current_and_set_gripper(-1.0, steps=total_steps)
        demo.refresh_runtime_handles(rebuild_visual=False)
        sync_planner_qpos_from_demo(demo)
        ...
        if obj is not None:
            obj_pose = obj.pose
            obj_p = np.round(flatten_np(obj_pose.p)[:3], 6)
            print(f"[sim release] {label} settled for {total_steps} steps, object p={obj_p.tolist()}")
        else:
            print(f"[sim release] {label} settled for {total_steps} steps")
    except Exception as exc:
        print(f"[warn] failed to settle the released object after gripper open: {exc}")
...
def validate_joint_path_detour_budget(demo, q_start: np.ndarray, q_path, *, label: str) -> bool:
    demo_args = getattr(demo, "args", None)
    max_ratio = float(max(getattr(demo_args, "rrt_max_detour_ratio", 1.8), 0.0)) if demo_args is not None else 1.8
    max_extra = float(max(getattr(demo_args, "rrt_max_detour_extra", 0.8), 0.0)) if demo_args is not None else 0.8
    if max_ratio <= 0.0 and max_extra <= 0.0:
        return True
    ...
    metrics = _joint_path_quality_metrics(q_start, q_path)
    direct_distance = float(np.linalg.norm(q_path[-1] - q_start))
    total_motion = float(metrics["total_motion"])

    allowed_candidates = []
    if max_ratio > 0.0:
        allowed_candidates.append(max_ratio * max(direct_distance, 1e-6))
    if max_extra > 0.0:
        allowed_candidates.append(direct_distance + max_extra)
    allowed_total = max(allowed_candidates) if allowed_candidates else float("inf")
    tol = 1e-3
    if total_motion <= allowed_total + tol:
        return True

    print(
        f"[planner] {label} rejected by detour budget: "
        f"total_motion={total_motion:.3f} rad, direct_distance={direct_distance:.3f} rad, "
        f"allowed_total={allowed_total:.3f} rad "
        f"(ratio_limit={max_ratio:.2f}, extra_limit={max_extra:.3f})"
    )
    return False


def _infer_motion_phase(label: str | None) -> str:
    label_l = str(label or "").strip().lower()
    if label_l.startswith("pre_place_staging"):
        return "pre_place_staging"
    if label_l.startswith("pre_place"):
        return "pre_place"
    if label_l.startswith("place_direct"):
        return "place_final"
    if label_l.startswith("place"):
        return "place_final"
    if "insert" in label_l:
        return "insert_vertical"
    if label_l.startswith("post_grasp_escape") or "transport" in label_l or "fixed_goal" in label_l:
        return "carry_transport"
    if label_l.startswith("grasp"):
        return "final_grasp_descent"
    if label_l.startswith("pregrasp") or "approach" in label_l:
        return "pregrasp"
    if label_l.startswith("sim_start"):
        return "carry_transport"
    return "generic"


def _ensure_motion_phase_context(demo, q_ref: np.ndarray | None = None) -> None:
    if getattr(demo, "_motion_phase_success_q", None) is None:
        demo._motion_phase_success_q = {}
    if getattr(demo, "_motion_phase_template_q", None) is not None:
        return

    if q_ref is None:
        q_ref = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    else:
        q_ref = np.asarray(q_ref, dtype=np.float32).reshape(-1)[:7]
    q_ref = retarget_goal_q_to_nearest_equivalent(demo, q_ref, q_ref)

    carry_template = q_ref.copy()
    demo_args = getattr(demo, "args", None)
    fixed_goal_deg = getattr(demo_args, "fixed_goal_joints_deg", None) if demo_args is not None else None
    if fixed_goal_deg is not None:
        try:
            q_fixed = np.deg2rad(np.asarray(fixed_goal_deg, dtype=np.float32).reshape(-1)[:7])
            carry_template = retarget_goal_q_to_nearest_equivalent(demo, q_fixed, q_ref)
        except Exception:
            carry_template = q_ref.copy()

    demo._motion_phase_template_q = {
        "generic": q_ref.copy(),
        "pregrasp": q_ref.copy(),
        "final_grasp_descent": q_ref.copy(),
        "carry_transport": carry_template.copy(),
        "pre_place_staging": carry_template.copy(),
        "pre_place": carry_template.copy(),
        "place_final": carry_template.copy(),
        "insert_vertical": carry_template.copy(),
    }


def _get_motion_phase_seed_qs(demo, phase: str, q_start: np.ndarray):
    q_start = np.asarray(q_start, dtype=np.float32).reshape(-1)[:7]
    _ensure_motion_phase_context(demo, q_start)

    phase = str(phase or "generic")
    success_cache = getattr(demo, "_motion_phase_success_q", {}) or {}
    template_cache = getattr(demo, "_motion_phase_template_q", {}) or {}

    seeds = [q_start.copy()]
    q_success = success_cache.get(phase)
    if q_success is not None:
        seeds.append(np.asarray(q_success, dtype=np.float32).reshape(-1)[:7])
    q_template = template_cache.get(phase)
    if q_template is not None:
        seeds.append(np.asarray(q_template, dtype=np.float32).reshape(-1)[:7])

    dedup = []
    seen = set()
    for q in seeds:
        q = retarget_goal_q_to_nearest_equivalent(demo, q, q_start)
        key = tuple(np.round(q, 5).tolist())
        if key in seen:
            continue
        seen.add(key)
        dedup.append(q.astype(np.float32))
    return dedup


def _joint_limit_proximity_penalty(demo, q: np.ndarray) -> float:
    joint_limits = np.asarray(getattr(demo.planner, "joint_limits", []), dtype=np.float32)
    q = np.asarray(q, dtype=np.float32).reshape(-1)[:7]
    ...
    for idx, q_i in enumerate(q):
        q_min = float(joint_limits[idx, 0])
        q_max = float(joint_limits[idx, 1])
        span = q_max - q_min
        if not np.isfinite(span) or span <= 1e-5:
            continue
        center = 0.5 * (q_min + q_max)
        half_span = max(0.5 * span, 1e-5)
        normalized = abs(float(q_i) - center) / half_span
        penalties.append(max(0.0, normalized - 0.8) ** 2)
    if not penalties:
        return 0.0
    return float(np.mean(penalties))


def _score_pose_goal_q_candidate(
    demo,
    q: np.ndarray,
    q_start: np.ndarray,
    *,
    ...
    dq = q - q_start
    total_norm = float(np.linalg.norm(dq))
    ...
    limit_penalty = _joint_limit_proximity_penalty(demo, q)

    success_cache = getattr(demo, "_motion_phase_success_q", {}) or {}
    template_cache = getattr(demo, "_motion_phase_template_q", {}) or {}
    q_family = success_cache.get(phase, template_cache.get(phase))
    template_delta = 0.0
    if q_family is not None:
        q_family = np.asarray(q_family, dtype=np.float32).reshape(-1)[:7]
        q_family = retarget_goal_q_to_nearest_equivalent(demo, q_family, q_start)
        template_delta = float(np.linalg.norm(q - q_family))

    return float(
        float(getattr(demo_args, "pose_ik_score_joint_distance_weight", 1.0)) * total_norm
        + float(getattr(demo_args, "pose_ik_score_joint7_weight", 0.20)) * joint7_delta
        + float(getattr(demo_args, "pose_ik_score_wrist_weight", 0.12)) * wrist_delta
        + float(getattr(demo_args, "pose_ik_score_limit_weight", 0.35)) * limit_penalty
        + float(getattr(demo_args, "pose_ik_score_template_weight", 0.08)) * template_delta
    )


def _record_motion_phase_success_q(demo, label: str, q: np.ndarray) -> None:
    q = np.asarray(q, dtype=np.float32).reshape(-1)[:7]
    phase = _infer_motion_phase(label)
    _ensure_motion_phase_context(demo, q)
    getattr(demo, "_motion_phase_success_q", {})[phase] = q.copy()
...
def _joint_path_quality_score(metrics: dict) -> float:
    return float(
        ...
        + 0.02 * metrics["joint7_total_motion"]
        + 0.01 * metrics["joint7_max_excursion"]
        + 0.005 * metrics["max_step_norm"]
        + 0.001 * metrics["waypoint_count"]
    )
...
def _select_best_rrt_success_candidate(label: str, q_start: np.ndarray, candidates):
    ...
    min_total_motion = min(float(item["metrics"]["total_motion"]) for item in scored_candidates)
    total_motion_band = max(0.25, 0.08 * min_total_motion)
    near_shortest = [
        item
        for item in scored_candidates
        if float(item["metrics"]["total_motion"]) <= min_total_motion + total_motion_band
    ]
    if len(near_shortest) < len(scored_candidates):
        kept_labels = [str(item["attempt_label"]) for item in near_shortest]
        print(
            f"[planner] {label} keeping only near-shortest candidates within "
            f"{total_motion_band:.3f} rad of the minimum total motion: {kept_labels}"
        )
    ...
    best = min(
        ...
        key=lambda item: (
            ...
            float(item["metrics"]["joint7_max_excursion"]),
            float(item["metrics"]["total_motion"]),
            float(item["metrics"]["max_step_norm"]),
...
def _sample_linear_joint_waypoints(q_start: np.ndarray, q_goal: np.ndarray, *, max_delta: float = 0.08):
    q_start = np.asarray(q_start, dtype=np.float32).reshape(-1)[:7]
    q_goal = np.asarray(q_goal, dtype=np.float32).reshape(-1)[:7]
    q_delta = q_goal - q_start
    max_delta = float(max(max_delta, 1e-3))
...
def _diagnose_linear_joint_path_quiet(
    demo,
    q_start: np.ndarray,
    q_goal: np.ndarray,
    *,
    ...
    q_start = np.asarray(q_start, dtype=np.float32).reshape(-1)[:7]
    q_goal = np.asarray(q_goal, dtype=np.float32).reshape(-1)[:7]
    samples = _sample_linear_joint_waypoints(q_start, q_goal, max_delta=max_delta)

    start_self = demo.planner.check_for_self_collision(qpos=q_start)
    start_env = demo.planner.check_for_env_collision(qpos=q_start, with_point_cloud=False, use_attach=use_attach)
    if (start_self or start_env) and not allow_start_in_collision:
        return None, 0, q_start, q_start, samples
    ...
    for idx, q in enumerate(samples):
        ...
        env_collisions = demo.planner.check_for_env_collision(qpos=q, with_point_cloud=False, use_attach=use_attach)
        if self_collisions or env_collisions:
            return None, idx, q_prev, q, samples
        q_path.append(q)
        q_prev = q
    return q_path, None, None, None, samples
...
def _plan_joint_path_rrt_only(
    ...
    *,
    ...
    dense_validate_delta = 0.015 if use_attach else 0.03
    ...
    def _run_rrt_attempts(reverse: bool = False):
        ...
        for attempt_name, attempt_time, attempt_range in attempt_configs:
            ...
            if not validate_joint7_turn_budget(demo, q_start, q_path, label=path_label):
                continue
            if not validate_joint_path_detour_budget(demo, q_start, q_path, label=path_label):
                continue
...
def _plan_joint_path_adaptive_segmented(
    demo,
    q_start: np.ndarray,
    q_goal: np.ndarray,
    *,
    ...
    linear_max_delta = 0.08 if use_attach else 0.12
    dense_validate_delta = 0.015 if use_attach else 0.03
    max_depth = int(max(getattr(args, "adaptive_segmented_joint_max_depth", 3), 0))
    leaf_max_delta = float(max(getattr(args, "adaptive_segmented_joint_leaf_max_delta", 0.25), 1e-3))
    leaf_attempt_count = int(max(getattr(args, "adaptive_segmented_joint_leaf_attempt_count", 2), 1))
    leaf_time_scale = float(max(getattr(args, "adaptive_segmented_joint_leaf_planning_time_scale", 0.5), 1e-3))
    leaf_planning_time = float(max(planning_time * leaf_time_scale, 0.5))

    def _choose_split_waypoint(samples, q_a: np.ndarray, q_b: np.ndarray):
        if not samples:
            return None, None
        mid_idx = max(0, (len(samples) - 1) // 2)
        q_mid = np.asarray(samples[mid_idx], dtype=np.float32).reshape(-1)[:7]
        q_mid = retarget_goal_q_to_nearest_equivalent(demo, q_mid, q_a)
        if np.allclose(q_mid, q_a, atol=1e-5) or np.allclose(q_mid, q_b, atol=1e-5):
            return None, None
        return q_mid, mid_idx

    def _solve_segment(q_a: np.ndarray, q_b: np.ndarray, depth: int, segment_label: str, allow_start_coll: bool):
        q_a = np.asarray(q_a, dtype=np.float32).reshape(-1)[:7]
        q_b = retarget_goal_q_to_nearest_equivalent(demo, np.asarray(q_b, dtype=np.float32).reshape(-1)[:7], q_a)
        if np.allclose(q_a, q_b, atol=1e-5):
            return []

        linear_path, fail_idx, _, _, samples = _diagnose_linear_joint_path_quiet(
            demo,
            q_a,
            q_b,
            use_attach=use_attach,
            max_delta=linear_max_delta,
            allow_start_in_collision=allow_start_coll,
        )
        if linear_path is not None:
            print(
                f"[planner] {segment_label} adaptive depth={depth} solved linearly "
                f"with {len(linear_path)} waypoint(s)"
            )
            return linear_path

        if fail_idx is not None and fail_idx > 0 and samples:
            safe_prefix = [
                np.asarray(q, dtype=np.float32).reshape(-1)[:7]
                for q in samples[:fail_idx]
            ]
            q_prefix_end = np.asarray(safe_prefix[-1], dtype=np.float32).reshape(-1)[:7]
            print(
                f"[planner] {segment_label} adaptive depth={depth} keeping linear-safe prefix "
                f"({len(safe_prefix)} waypoint(s)) and repairing the remaining tail"
            )
            tail_path = _solve_segment(
                q_prefix_end,
                q_b,
                depth,
                f"{segment_label}_tail",
                False,
            )
            if tail_path is None:
                return None
            return safe_prefix + list(tail_path)

        max_abs_delta = float(np.max(np.abs(q_b - q_a))) if q_a.size else 0.0
        use_leaf_rrt = depth >= max_depth or max_abs_delta <= leaf_max_delta
        if use_leaf_rrt:
            print(
                f"[planner] {segment_label} adaptive depth={depth} switching to local RRT leaf "
                f"(max_abs_delta={max_abs_delta:.3f} rad, fail_sample={None if fail_idx is None else fail_idx + 1})"
            )
            return _plan_joint_path_rrt_only(
                demo,
                q_b,
                use_attach=use_attach,
                label=f"{segment_label}_leaf",
                planning_time=leaf_planning_time,
                rrt_range=rrt_range,
                start_q=q_a,
                allow_linear_fallback=False,
                prefer_linear_first=False,
                allow_reverse_rrt_fallback=False,
                prefer_reverse_rrt=False,
                allow_shortcut=allow_shortcut,
                allow_start_in_collision=allow_start_coll,
                attempt_count_override=leaf_attempt_count,
                announce_start_goal=False,
            )

        q_mid, mid_idx = _choose_split_waypoint(samples, q_a, q_b)
        if q_mid is None:
            print(f"[planner] {segment_label} adaptive depth={depth} found no valid split waypoint; using local RRT leaf")
            return _plan_joint_path_rrt_only(
                demo,
                q_b,
                use_attach=use_attach,
        ...
        print(
            f"[planner] {segment_label} adaptive depth={depth} splitting at sampled midpoint "
            f"{mid_idx + 1}/{len(samples)} after linear failure"
        )
        left_path = _solve_segment(q_a, q_mid, depth + 1, f"{segment_label}_L", allow_start_coll)
        if left_path is None:
            return None
        q_mid_actual = np.asarray(left_path[-1], dtype=np.float32).reshape(-1)[:7] if left_path else q_a
        right_path = _solve_segment(q_mid_actual, q_b, depth + 1, f"{segment_label}_R", False)
        if right_path is None:
            return None
        return list(left_path) + list(right_path)

    print(
        f"[planner] {label} trying adaptive segmented joint planning "
        f"(max_depth={max_depth}, leaf_max_delta={leaf_max_delta:.3f} rad)"
    )
    adaptive_path = _solve_segment(q_start, q_goal, 0, label, allow_start_in_collision)
    if adaptive_path is None:
        print(f"[planner] {label} adaptive segmented planning failed")
        return None

    final_path = [np.asarray(q, dtype=np.float32).reshape(-1)[:7] for q in adaptive_path]
    if allow_shortcut and final_path:
        shortcut_path = shortcut_joint_path(
            demo,
            q_start,
            final_path,
            use_attach=use_attach,
            label=f"{label}_adaptive",
            max_delta=linear_max_delta,
        )
        if shortcut_path is not final_path and not validate_joint_path_segments(
            demo,
            q_start,
            shortcut_path,
            use_attach=use_attach,
            label=f"{label}_adaptive_shortcut",
            max_delta=dense_validate_delta,
            allow_start_in_collision=allow_start_in_collision,
        ):
            print(f"[planner] {label} adaptive shortcut rejected by dense validation; keeping the unshortened adaptive path")
        else:
            final_path = shortcut_path

    if not validate_joint7_turn_budget(demo, q_start, final_path, label=f"{label}_adaptive_final"):
        print(f"[planner] {label} adaptive segmented plan failed final turn-budget validation")
        return None
    if not validate_joint_path_detour_budget(demo, q_start, final_path, label=f"{label}_adaptive_final"):
        print(f"[planner] {label} adaptive segmented plan failed final detour validation")
        return None
    if not validate_joint_path_segments(
        demo,
        q_start,
        final_path,
        use_attach=use_attach,
        label=f"{label}_adaptive_final",
        max_delta=dense_validate_delta,
        allow_start_in_collision=allow_start_in_collision,
    ):
        print(f"[planner] {label} adaptive segmented plan failed final dense validation")
        return None
    print(f"[planner] {label} adaptive segmented planning succeeded with {len(final_path)} waypoint(s)")
    return final_path
...
def plan_joint_path(
    ...
    *,
    ...
    adaptive_enabled = bool(getattr(getattr(demo, "args", None), "adaptive_segmented_joint_planning", True))
    if adaptive_enabled and allow_linear_fallback and not np.allclose(q_start, q_goal, atol=1e-5):
        adaptive_path = _plan_joint_path_adaptive_segmented(
            demo,
            q_start,
            q_goal,
            use_attach=use_attach,
        ...
        print(f"[planner] {label} adaptive segmented planner failed; falling back to direct global RRT")
    ...
    return _plan_joint_path_rrt_only(
        ...
        prefer_linear_first=prefer_linear_first,
...
def _solve_pose_goal_q_candidates(
    demo,
    pose,
    q_start: np.ndarray,
    *,
    ...
    q_start = np.asarray(q_start, dtype=np.float32).reshape(-1)[:7]
    ik_target = demo.make_target(pose, variant_name)
    phase = _infer_motion_phase(label) if phase is None else str(phase or "generic")
    q_seed_list = _get_motion_phase_seed_qs(demo, phase, q_start)
    top_k = int(
        max(
            1,
            min(
                int(max_candidates),
                int(max(getattr(getattr(demo, "args", None), "pose_ik_candidate_top_k", max_candidates), 1)),
            ),
        )
    )

    thresholds = [float(max(strict_threshold, 1e-5))]
    relaxed_threshold = float(max(relaxed_threshold, thresholds[0]))
    if relaxed_threshold > thresholds[0] + 1e-6:
        thresholds.append(relaxed_threshold)

    last_status = None
    for threshold in thresholds:
        if verbose:
            if threshold <= thresholds[0] + 1e-6:
                print(f"[planner] {label} solving pose IK for joint-goal candidates")
            else:
                print(f"[planner] {label} retrying pose IK with relaxed threshold={threshold:.4f} m")

        dedup = {}
        threshold_statuses = []
        for q_seed in q_seed_list:
            status, ik_results = demo.planner.IK(ik_target, np.asarray(q_seed, dtype=np.float32).reshape(-1)[:7], threshold=threshold)
            threshold_statuses.append(str(status))
            last_status = status
            if status != "Success":
                continue
            for q in list(ik_results or []):
                q = np.asarray(q, dtype=np.float32).reshape(-1)[:7]
                q = retarget_goal_q_to_nearest_equivalent(demo, q, q_start)
                key = tuple(np.round(q, 5).tolist())
                if key not in dedup:
        ...
        if verbose:
            success_count = sum(1 for status in threshold_statuses if status == "Success")
            status_label = "IK status" if threshold <= thresholds[0] + 1e-6 else "relaxed IK status"
            if dedup:
                print(
                    f"[planner] {label} {status_label}: Success "
                    f"({len(dedup)} dedup candidate(s) from {success_count}/{len(q_seed_list)} seed(s))"
                )
            else:
                print(
                    f"[planner] {label} {status_label}: "
                    f"{last_status if last_status is not None else 'IK Failed'}"
                )
        if not dedup:
            continue
        ...
        sorted_candidates = sorted(
            dedup.values(),
            key=lambda q: _score_pose_goal_q_candidate(
                demo,
                np.asarray(q, dtype=np.float32).reshape(-1)[:7],
                q_start,
                phase=phase,
        ...
        return [np.asarray(q, dtype=np.float32).reshape(-1)[:7] for q in sorted_candidates[:top_k]]

    if verbose:
        print(f"[planner] {label} failed before joint planning because pose IK did not return any goal joint solution")
        if last_status is not None:
            print(f"[planner] {label} last IK status: {last_status}")
    return None


def _plan_pose_path_rrt_only(demo, pose, *, variant_name: str = "array_wxyz", use_attach: bool = False, label: str = "pose_goal", planning_time: float = 4.0, rrt_range: float = 0.15, start_q=None, allow_start_in_collision: bool = False, attempt_count_override: int | None = None):
    q_start = np.asarray(demo.current_arm_qpos() if start_q is None else start_q, dtype=np.float32).reshape(-1)[:7]
    ...
    for attempt_name, attempt_time, attempt_range in attempt_configs:
        ...
        if not validate_joint7_turn_budget(demo, q_start, q_path, label=label):
            continue
        if not validate_joint_path_detour_budget(demo, q_start, q_path, label=label):
            continue
        if not validate_joint_path_segments(
            demo,
            q_start,
...
def _quat_angle_between_wxyz(q0, q1) -> float:
...
def _should_use_segmented_pose_rrt(label: str) -> bool:
    label_l = str(label or "").strip().lower()
    if label_l.startswith("grasp"):
        return False
    if label_l.startswith("pregrasp"):
        return False
    if "_to_true_grasp" in label_l:
        return False
    return True


def _plan_pose_path_segmented_rrt(
    demo,
    pose,
    *,
    variant_name: str = "array_wxyz",
    ...
    args = getattr(demo, "args", None)
    if not bool(getattr(args, "segmented_pose_rrt", False)):
        return None
    if not _should_use_segmented_pose_rrt(label):
        return None
    ...
    max_translation = float(max(getattr(args, "segmented_pose_max_translation", 0.18), 1e-3))
    max_rotation_rad = float(np.deg2rad(max(getattr(args, "segmented_pose_max_rotation_deg", 35.0), 1.0)))
    trans_dist = float(np.linalg.norm(p_goal - p_start))
    rot_dist = _quat_angle_between_wxyz(q_start_pose, q_goal_pose)
    num_segments = max(
        int(np.ceil(trans_dist / max_translation)),
        int(np.ceil(rot_dist / max_rotation_rad)),
    )
    if num_segments <= 1:
        return None

    per_segment_planning_time = float(max(planning_time * float(max(getattr(args, "segmented_pose_rrt_planning_time_scale", 0.75), 1e-3)), 1.5))
    per_segment_attempt_count = int(max(getattr(args, "segmented_pose_rrt_attempt_count", 3), 1))
    print(
        f"[planner] {label} trying segmented pose-RRT with {num_segments} segment(s) "
        f"(translation={trans_dist:.3f} m, rotation={np.rad2deg(rot_dist):.1f} deg)"
    )

    q_cursor = q_start.copy()
    q_path_full: list[np.ndarray] = []
    for seg_idx, alpha in enumerate(np.linspace(0.0, 1.0, num_segments + 1, dtype=np.float32)[1:], start=1):
        p_target = ((1.0 - alpha) * p_start + alpha * p_goal).astype(np.float32)
        q_target = _slerp_quat_wxyz(q_start_pose, q_goal_pose, float(alpha))
        pose_target = Pose.create_from_pq(p=p_target, q=q_target)
        seg_label = f"{label}_seg{seg_idx}"
        q_goal_candidates = _solve_pose_goal_q_candidates(
            demo,
            pose_target,
            q_cursor,
        ...
        if q_goal_candidates:
            seg_success_candidates = []
            for ik_idx, q_goal in enumerate(q_goal_candidates, start=1):
                q_seg_path = plan_joint_path(
                    demo,
                    q_goal,
                    use_attach=use_attach,
                    label=f"{seg_label}_ik{ik_idx}",
                    planning_time=per_segment_planning_time,
                    rrt_range=rrt_range,
                    start_q=q_cursor,
                    ...
                    attempt_count_override=per_segment_attempt_count,
                )
                if q_seg_path is None:
                    continue
                seg_success_candidates.append(
                    {
                        "attempt_label": f"ik{ik_idx}",
                        "path": q_seg_path,
                    }
                )
            if seg_success_candidates:
                best_candidate = _select_best_rrt_success_candidate(seg_label, q_cursor, seg_success_candidates)
                seg_path = best_candidate["path"]
        if seg_path is None:
            seg_path = _plan_pose_path_rrt_only(
                demo,
                pose_target,
                variant_name=variant_name,
                use_attach=use_attach,
        ...
        if seg_path is None:
            print(f"[planner] {label} segmented pose-RRT failed at segment {seg_idx}/{num_segments}")
            return None
        q_path_full.extend([np.asarray(q, dtype=np.float32).reshape(-1)[:7] for q in seg_path])
        if q_path_full:
            q_cursor = np.asarray(q_path_full[-1], dtype=np.float32).reshape(-1)[:7]

    if not validate_joint7_turn_budget(demo, q_start, q_path_full, label=f"{label}_segmented_final"):
        print(f"[planner] {label} segmented pose-RRT failed final turn-budget validation")
        return None
    if not validate_joint_path_detour_budget(demo, q_start, q_path_full, label=f"{label}_segmented_final"):
        print(f"[planner] {label} segmented pose-RRT failed final detour validation")
        return None
    if not validate_joint_path_segments(
        demo,
        q_start,
        q_path_full,
        use_attach=use_attach,
        label=f"{label}_segmented_final",
        max_delta=0.015 if use_attach else 0.03,
        allow_start_in_collision=allow_start_in_collision,
    ):
        print(f"[planner] {label} segmented pose-RRT failed final dense validation")
        return None
    print(f"[planner] {label} segmented pose-RRT succeeded with {len(q_path_full)} waypoint(s)")
    return q_path_full
...
def plan_pose_path(demo, pose, *, variant_name: str = "array_wxyz", use_attach: bool = False, label: str = "pose_goal", planning_time: float = 4.0, rrt_range: float = 0.15, start_q=None, allow_start_in_collision: bool = False):
    ...
    segmented_path = _plan_pose_path_segmented_rrt(
        demo,
        pose,
        variant_name=variant_name,
        use_attach=use_attach,
    ...
    return _plan_pose_path_rrt_only(
        demo,
        pose,
        variant_name=variant_name,
        use_attach=use_attach,
...
def plan_grasp_descent_path(
    ...
    *,
    ...
    if not bool(getattr(args, "grasp_local_descent", True)):
        return plan_pose_path(
            demo,
            pose,
            variant_name=variant_name,
    ...
    local_path = plan_local_cartesian_lift_path(
        ...
        label=label,
        start_q=start_q,
        max_cartesian_step=0.0025,
        max_segment_joint_delta=0.18,
        max_segment_joint7_delta=0.20,
        max_segment_norm_delta=0.25,
        allow_start_in_collision=False,
    )
    if local_path:
        return local_path

    if not bool(getattr(args, "grasp_local_descent_allow_pose_rrt_fallback", False)):
        print(f"[planner] {label} local Cartesian grasp descent failed; pose RRT fallback disabled to avoid in-descent wrist rotation")
        return None

    print(f"[planner] {label} local Cartesian grasp descent failed; falling back to pose RRT")
    return plan_pose_path(
        demo,
        pose,
        variant_name=variant_name,
...
def _clip_arm_q_to_joint_limits(demo, q: np.ndarray) -> np.ndarray:
    q = np.asarray(q, dtype=np.float32).reshape(-1)[:7].copy()
    joint_limits = np.asarray(getattr(demo.planner, "joint_limits", []), dtype=np.float32)
    if joint_limits.ndim != 2 or joint_limits.shape[0] < q.shape[0] or joint_limits.shape[1] < 2:
        return q
    q_min = joint_limits[: q.shape[0], 0] + 1e-4
    q_max = joint_limits[: q.shape[0], 1] - 1e-4
    return np.minimum(np.maximum(q, q_min), q_max).astype(np.float32)


def _estimate_tcp_position_jacobian(demo, q_ref: np.ndarray, *, eps: float = 0.04):
    q_ref = _clip_arm_q_to_joint_limits(demo, np.asarray(q_ref, dtype=np.float32).reshape(-1)[:7])
    q_saved = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    joint_limits = np.asarray(getattr(demo.planner, "joint_limits", []), dtype=np.float32)
    J = np.zeros((3, q_ref.shape[0]), dtype=np.float32)
    try:
        sync_demo_arm_qpos(demo, q_ref)
        p_ref = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
        for joint_idx in range(q_ref.shape[0]):
            if joint_limits.ndim == 2 and joint_limits.shape[0] > joint_idx and joint_limits.shape[1] >= 2:
                q_min = float(joint_limits[joint_idx, 0])
                q_max = float(joint_limits[joint_idx, 1])
            else:
                q_min = float(q_ref[joint_idx] - eps)
                q_max = float(q_ref[joint_idx] + eps)

            step_plus = min(float(eps), max(0.0, q_max - float(q_ref[joint_idx]) - 1e-4))
            step_minus = min(float(eps), max(0.0, float(q_ref[joint_idx]) - q_min - 1e-4))

            if step_plus > 1e-4 and step_minus > 1e-4:
                q_plus = q_ref.copy()
                q_minus = q_ref.copy()
                q_plus[joint_idx] += step_plus
                q_minus[joint_idx] -= step_minus
                sync_demo_arm_qpos(demo, q_plus)
                p_plus = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
                sync_demo_arm_qpos(demo, q_minus)
                p_minus = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
                J[:, joint_idx] = (p_plus - p_minus) / float(step_plus + step_minus)
            elif step_plus > 1e-4:
                q_plus = q_ref.copy()
                q_plus[joint_idx] += step_plus
                sync_demo_arm_qpos(demo, q_plus)
                p_plus = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
                J[:, joint_idx] = (p_plus - p_ref) / float(step_plus)
            elif step_minus > 1e-4:
                q_minus = q_ref.copy()
                q_minus[joint_idx] -= step_minus
                sync_demo_arm_qpos(demo, q_minus)
                p_minus = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
                J[:, joint_idx] = (p_ref - p_minus) / float(step_minus)
        return p_ref, J
    finally:
        sync_demo_arm_qpos(demo, q_saved)


def _plan_joint_space_tcp_escape_path(
    demo,
    *,
    ...
    q_start = _clip_arm_q_to_joint_limits(demo, np.asarray(start_q, dtype=np.float32).reshape(-1)[:7])
    tcp_pose = demo.tcp.pose
    tcp_p = flatten_np(tcp_pose.p)[:3].astype(np.float32)
    escape_xy = None
    if float(retreat_distance) > 1e-6:
        escape_xy = _nearest_obstacle_escape_xy(demo, tcp_p[:2])
        if escape_xy is None:
            escape_xy = np.array([1.0, 0.0], dtype=np.float32)
    desired_delta = np.array(
        [
            0.0 if escape_xy is None else float(escape_xy[0]) * float(retreat_distance),
            0.0 if escape_xy is None else float(escape_xy[1]) * float(retreat_distance),
            float(lift_height),
        ],
        dtype=np.float32,
    )
    ...
    p_start_ref = None
    for _ in range(2):
        p_ref, J = _estimate_tcp_position_jacobian(demo, q_goal, eps=0.04)
        if p_start_ref is None:
            p_start_ref = p_ref.copy()
        achieved_so_far = p_ref - p_start_ref
        remaining = desired_delta - achieved_so_far
        remaining_norm = float(np.linalg.norm(remaining))
        if remaining_norm <= 0.005:
            break
        damping = 0.05
        try:
            dq = J.T @ np.linalg.solve(J @ J.T + (damping ** 2) * np.eye(3, dtype=np.float32), remaining)
        except np.linalg.LinAlgError:
            return None
        dq = np.asarray(dq, dtype=np.float32).reshape(-1)[:7]
        max_abs = float(np.max(np.abs(dq))) if dq.size else 0.0
        norm_dq = float(np.linalg.norm(dq))
        if max_abs <= 1e-6 or norm_dq <= 1e-6:
            return None
        scale = 1.0
        if max_abs > 0.45:
            scale = min(scale, 0.45 / max_abs)
        if norm_dq > 0.90:
            scale = min(scale, 0.90 / norm_dq)
        q_goal = _clip_arm_q_to_joint_limits(demo, q_goal + dq * float(scale))

    q_saved = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    try:
        sync_demo_arm_qpos(demo, q_start)
        p_start = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
        sync_demo_arm_qpos(demo, q_goal)
        p_goal = flatten_np(demo.tcp.pose.p)[:3].astype(np.float32)
    finally:
        sync_demo_arm_qpos(demo, q_saved)

    achieved_delta = p_goal - p_start
    progress_along = float(np.dot(achieved_delta, desired_delta / max(desired_norm, 1e-6)))
    print(
        f"[planner] {label} joint-space escape candidate: "
        f"desired_delta={np.round(desired_delta, 4)}, achieved_delta={np.round(achieved_delta, 4)}, "
        f"progress_along={progress_along:.4f} m"
    )
    if progress_along < max(0.01, 0.35 * desired_norm):
        print(f"[planner] {label} joint-space escape rejected: insufficient progress along the desired escape direction")
        return None
    if np.allclose(q_goal, q_start, atol=1e-4):
        return None
...
def execute_joint_path_stage(demo, bridge_mod, real_exec: RealmanJointExecutor | None, label: str, q_path, gripper_pos: float, args, *, use_attach: bool = False, allow_start_in_collision: bool = False):
    ...
    if not q_path:
        ...
        _record_motion_phase_success_q(demo, label, q_current)
    ...
    _record_motion_phase_success_q(demo, label, q_path[-1])
...
def retry_pregrasp_after_escape(
    ...
    for candidate_idx, (candidate_retreat_m, candidate_lift_m) in enumerate(candidates, start=1):
        ...
        pre_escape_max_joint_delta = float(max(getattr(args, "pregrasp_blocked_retry_max_segment_joint_delta", 0.60), 0.0))
        pre_escape_max_joint7_delta = float(max(getattr(args, "pregrasp_blocked_retry_max_segment_joint7_delta", 1.20), 0.0))
        pre_escape_max_norm_delta = float(max(getattr(args, "pregrasp_blocked_retry_max_segment_norm_delta", 1.00), 0.0))
        escape_path = plan_lift_path(
            ...
            rrt_range=0.15,
            allow_pose_rrt_fallback=True,
            max_segment_joint_delta=pre_escape_max_joint_delta,
            max_segment_joint7_delta=pre_escape_max_joint7_delta,
            max_segment_norm_delta=pre_escape_max_norm_delta,
            allow_start_in_collision=True,
        )
        ...
        if not ok_escape:
            ...
            print(f"[planner] {retry_label} pose-path planning failed after pose-based pre-escape; falling back to terminal joint target")
        ...
        print(f"[planner] {escape_label} pose-based pre-escape failed; trying joint-space escape fallback")
        joint_escape_path = _plan_joint_space_tcp_escape_path(
            demo,
            retreat_distance=candidate_retreat_m,
            lift_height=candidate_lift_m,
            label=escape_label,
            start_q=np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7],
            planning_time=3.0,
            rrt_range=0.15,
        ...
        if joint_escape_path is None or len(joint_escape_path) == 0:
            print(
                f"[warn] {escape_label}_joint planning failed for retreat={candidate_retreat_m:.3f} m, "
                f"lift={candidate_lift_m:.3f} m"
            )
            continue

        ok_escape, _ = execute_joint_path_stage(
            demo,
            bridge_mod,
            real_exec,
            f"{escape_label}_joint",
            joint_escape_path,
            gripper_pos,
            args,
            use_attach=False,
        ...
        if not ok_escape:
            print(f"[warn] {escape_label}_joint execution failed")
            continue
...
def update_scene_capture_cache_object(demo, object_name: str, T_cam_obj: np.ndarray, object_args) -> None:
    ...
    objects[normalized] = {
        ...
        "placed": bool(old_entry.get("placed", False)),
    }
...
def apply_refined_target_pose_from_camera(demo, bridge_mod, T_cam_obj: np.ndarray, args) -> None:
    ...
    demo.refresh_runtime_handles(rebuild_visual=False)
    min_clearance = max(0.001, 0.5 * float(getattr(args, "min_object_center_z_margin", 0.0)))
    if bool(getattr(args, "lock_object_z_to_table", True)):
        lock_active_object_z_to_table_if_needed(
            demo,
            args,
            min_clearance=min_clearance,
        )
...
def apply_refined_scene_obstacle_pose_from_camera(demo, bridge_mod, object_name: str, T_cam_obj: np.ndarray, object_args) -> None:
    ...
    if bool(getattr(object_args, "lock_object_z_to_table", True)):
        lock_scene_obstacles_z_to_table_if_needed(
            demo.env,
            demo,
            object_args,
            min_clearance=min_clearance,
...
def create_demo(args, bridge_mod, planner_mod, scene_capture_cache=None):
    ...
    demo._frozen_active_object_pose = None
    demo._motion_phase_success_q = {}
    demo._motion_phase_template_q = None
    demo._cycle_start_arm_q = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
    _ensure_motion_phase_context(demo, demo._cycle_start_arm_q)
    min_table_clearance = max(0.001, 0.5 * float(getattr(args, "min_object_center_z_margin", 0.0)))
    snap_active_object_flat_on_table_if_needed(
        demo,
        args,
        min_clearance=min_table_clearance,
    )
    if bool(getattr(args, "lock_object_z_to_table", True)):
        lock_active_object_z_to_table_if_needed(
            demo,
...
def execute_stage(demo, bridge_mod, real_exec: RealmanJointExecutor | None, label: str, pose, q_target: np.ndarray, gripper_pos: float, args):
    ...
    _record_motion_phase_success_q(demo, label, q_target)
...
def run_real_episode(demo, bridge_mod, real_exec: RealmanJointExecutor | None, args) -> bool:
    ...
    if bool(getattr(grasp_plan, "require_runtime_true_grasp_planning", False)):
        ...
        q_grasp_path = plan_grasp_descent_path(
            demo,
            selected_grasp_pose,
            args,
            variant_name=args.variant,
        ...
        if q_grasp_path is not None:
            ...
            if bool(getattr(args, "grasp_local_descent", True)):
                print(f"[FAIL] {grasp_label}_to_true_grasp local grasp descent planning failed from the executed fallback approach")
        ...
        if q_grasp_path is not None:
            ...
            print(f"[planner] {grasp_label}_to_true_grasp local grasp descent planning failed")
            ok = False
    else:
        q_grasp_path = plan_grasp_descent_path(
            demo,
        ...
        if q_grasp_path is not None:
            ...
            if bool(getattr(args, "grasp_local_descent", True)):
                print(f"[FAIL] {grasp_label or 'grasp'} local grasp descent planning failed")
                inspect_failed_pose(demo, bridge_mod, "grasp", args, pose=selected_grasp_pose, gripper_closed=False)
                return False
            print(f"[planner] {grasp_label or 'grasp'} pose-path planning failed; falling back to terminal joint target")
            ok, _ = execute_stage(
    ...
    demo._attached_object_visual_active = False
    update_attached_box_visual(demo, visible=False)
    settle_released_object_after_open(demo, args, label="fixed_goal_release")
    ...
    try:
        if args.execute_real:
            ...
            if args.reset_real_before_start:
                ...
                if not confirm_simple_action("reset the real robot to its hardware home pose before FoundationPose initialization", args):
