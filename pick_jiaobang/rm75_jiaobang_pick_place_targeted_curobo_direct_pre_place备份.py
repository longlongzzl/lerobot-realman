...
def build_arg_parser():
    ...
    parser.set_defaults(
        ...
        insert_vertical_axial_spin_deg=[0.0, 90.0, 180.0, 270.0],
        targeted_place_hover_extra_height_m=[0.0, 0.02, 0.04],
        tabletop_place_tilt_toward_robot_deg=[0.0],
        tabletop_place_yaw_variant_deg=[0.0, -30.0, 30.0, -60.0, 60.0, -90.0, 90.0, 180.0],
        tabletop_place_axial_spin_deg=[0.0],
        topdown_grasp_yaw_variant_deg=[0.0],
        topdown_tilt_toward_robot_deg=[0.0],
        topdown_tilt_toward_robot_shift_m=[0.0],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="For tabletop place_on_slots rules, lift the direct-place planning target upward by this much, then plan a short final descent to the true contact place pose.",
    )
    parser.add_argument(
...
def _diagnose_direct_grasp_failure_sources(planner, demo, args, start_q, grasp_candidates):
    grasp_candidates = list(grasp_candidates or [])
    if not grasp_candidates:
        return
    print("[curobo][diag] probing direct_grasp failure source against active target object")
    base_disabled_links = _direct_grasp_target_contact_only_disabled_links(planner)
    no_target_successes = _evaluate_curobo_pose_candidates_goalset(
        planner,
        demo,
        args,
        start_q,
        grasp_candidates,
        label="direct_grasp_diag_without_target",
        prefer_verticality=False,
        use_attach=False,
        max_winners=1,
        include_active_object=False,
        disabled_world_collision_links=base_disabled_links,
    )
    if not no_target_successes:
        print(
            "[curobo][diag] no grasp candidate succeeded even after removing the active target obstacle; "
            "failure is not isolated to target-object collision."
        )
        return

    probe_candidate = dict(no_target_successes[0])
    probe_candidates = [{"label": str(probe_candidate["label"]), "pose": probe_candidate["pose"]}]
    print(
        f"[curobo][diag] candidate {probe_candidate['label']} succeeds without active target obstacle; "
        "probing which remaining links collide with the target."
    )
    probe_groups = [
        ("body_only", list(base_disabled_links)),
        ("body_plus_pads", sorted(set(base_disabled_links + ["left_pad", "right_pad"]))),
        (
            "body_plus_distal",
            sorted(set(base_disabled_links + ["gripper_Left_2_Link", "gripper_Right_2_Link"])),
        ),
        (
            "body_plus_pads_plus_distal",
            sorted(
                set(
                    base_disabled_links
                    + ["left_pad", "right_pad", "gripper_Left_2_Link", "gripper_Right_2_Link"]
                )
            ),
        ),
        (
            "body_plus_pads_plus_distal_plus_link7",
            sorted(
                set(
                    base_disabled_links
                    + [
    ...
    for probe_name, disabled_links in probe_groups:
        probe_successes = _evaluate_curobo_pose_candidates_goalset(
            planner,
            demo,
            args,
            start_q,
            probe_candidates,
            label=f"direct_grasp_diag_{probe_name}",
            prefer_verticality=False,
            use_attach=False,
            max_winners=1,
        ...
        if probe_successes:
            print(
                f"[curobo][diag] {probe_name} succeeded with disabled world-collision links: "
                f"{disabled_links}"
            )
            return
        print(
            f"[curobo][diag] {probe_name} still failed with disabled world-collision links: "
            f"{disabled_links}"
        )
    print(
        "[curobo][diag] even after disabling pads, distal fingers, and link_7 against the world, "
        "the representative grasp still failed. The remaining issue is more likely grasp pose "
        "geometry, the locked gripper kinematics, or a broader scene constraint."
    )
...
def _plan_short_curobo_cartesian_descent(
    ...
    *,
    ...
    print(
        ...
        f"(distance={distance:.4f} m, step={step_m:.4f} m)"
    )
...
def _plan_place_motion_from_pre_place(
    planner,
    demo,
    args,
    start_q,
    ...
    *,
    ...
    if str(primitive) == "insert_vertical":
        return _plan_short_curobo_cartesian_descent(
            planner,
            demo,
            args,
            start_q,
    ...
    print(f"[curobo] {label} planning full place motion from pre_place to place")
    result = planner.plan_to_pose(
        start_q,
        planner_pose,
        enable_graph=bool(getattr(args, "curobo_enable_graph", False)),
    ...
    if not result.success or result.joint_path is None:
        print(f"[curobo] {label} full place motion failed with status={result.status}")
        return None
    q_path = [np.asarray(q, dtype=np.float32).reshape(-1)[:7] for q in result.joint_path]
...
def _evaluate_curobo_pose_candidates_goalset(
    ...
    *,
    ...
    disabled_world_collision_links = [
        str(x) for x in list(disabled_world_collision_links or []) if str(x)
    ]
    if disabled_world_collision_links:
        planner.set_world_collision_for_links(disabled_world_collision_links, enabled=False)
        print(
            f"[curobo] {label} temporarily disabled world collision for links: "
            f"{disabled_world_collision_links}"
        )
    ...
    try:
        ...
        if disabled_world_collision_links:
            planner.set_world_collision_for_links(disabled_world_collision_links, enabled=True)
    ...
    *,
    ...
    for chunk_idx, start_idx in enumerate(range(0, len(remaining), chunk_size), start=1):
        ...
        for candidate, planner_pose, result in zip(chunk, planner_poses, batch_results):
            ...
            if not result.success or result.joint_path is None:
                print(f"[curobo] {candidate_label} batch failed with status={result.status}")
                status_key = "None" if result.status is None else str(result.status)
    ...
    if label.startswith("joint_direct_place_pairs") or label.startswith("joint_pre_place_pairs"):
        failure_count = int(sum(status_counts.values()))
        status_summary = ", ".join(
            f"{status}={count}"
            for status, count in sorted(status_counts.items(), key=lambda item: (-item[1], item[0]))
        )
        print(
            f"[curobo][diag] {label} result summary: "
            f"success={success_count}, failed={failure_count}"
            + (f", statuses: {status_summary}" if status_summary else "")
        )
...
def _build_direct_grasp_candidates(demo, args):
    ...
    def _build_pose_variants():
        ...
        for label, pose in targeted.base.build_grasp_pose_variants(demo, raw_grasp_pose, args):
            _append(label, pose)
        if grasp_mode in {"topdown_long_axis", "pen_topdown_insert_ready", "long_axis_adaptive"}:
            tilt_degs = _unique_finite_float_list(
                getattr(args, "direct_elongated_grasp_tilt_toward_robot_deg", [0.0]),
            )
            ...
            for tilt_deg in tilt_degs:
                ...
                for shift_d in shift_ds:
                    shifted_pose = (
                        tilted_pose
                        if abs(float(shift_d)) <= 1e-6
                        else targeted.base.shift_pose_toward_robot_xy(demo, tilted_pose, float(shift_d))
                    )
                    if shifted_pose is None:
                        continue
                    label = f"grasp_elong_tilt_{int(round(abs(float(tilt_deg))))}deg"
                    if abs(float(shift_d)) > 1e-6:
                        label += f"_shift_{int(round(float(shift_d) * 1000.0))}mm"
                    _append(label, shifted_pose)
        return variants
...
def _build_direct_pre_place_candidates(demo, bridge_mod, scene_capture_cache, rule, place_state_cache, args):
    ...
    if not z_offsets:
        z_offsets = [0.0]
    if str(rule.primitive) == "place_on_slots":
        z_offsets = [0.0]
...
def _build_direct_place_candidates(demo, bridge_mod, scene_capture_cache, rule, place_state_cache, args):
    ...
    min_place_tcp_z = float(getattr(args, "direct_min_place_tcp_z", 0.005))
    contact_lift_m = float(max(getattr(args, "direct_place_contact_lift_m", 0.0), 0.0))
    candidates = []
    for candidate in place_plan_candidates:
        place_z = float(_get_pose_position(candidate.place_pose)[2])
        if place_z < min_place_tcp_z:
        ...
        if str(rule.primitive) == "place_on_slots" and contact_lift_m > 1e-6:
            lifted_p = (_get_pose_position(candidate.place_pose) + np.array([0.0, 0.0, contact_lift_m], dtype=np.float32)).astype(np.float32)
            pose = targeted.base.make_pose_with_position(candidate.place_pose, lifted_p)
    ...
    print(
        ...
        f"(contact_lift_m={contact_lift_m:.3f})"
    )
    return candidates
...
def _evaluate_joint_grasp_place_chains(
    ...
    goalset_max_winners = int(getattr(args, "joint_search_goalset_max_winners", 0))
    use_direct_place_pairs = False
    try:
        pair_candidates = []
        for grasp_choice in grasp_candidates:
            ...
            if use_direct_place_pairs:
                ...
                if not direct_place_candidates:
                    print(f"[joint_search] {grasp_label}: no direct place candidate could be built")
                    continue
                ...
                for candidate in direct_place_candidates:
                    item = dict(candidate)
                    item["start_q"] = grasp_terminal_q.copy()
                    item["grasp_choice"] = grasp_choice
                    item["base_label"] = str(candidate["label"])
                    item["label"] = f"{grasp_label}__{candidate['label']}"
        ...
        if not pair_candidates:
            print("[joint_search] no grasp/place pair candidate could be built")
            return []
        unique_grasp_labels = sorted({str(item["grasp_choice"]["label"]) for item in pair_candidates})
        print(
            f"[joint_search] pair candidate count: {len(pair_candidates)} "
            f"from {len(unique_grasp_labels)} grasp winner(s) "
            f"(grasp_labels={unique_grasp_labels[:8]}{'...' if len(unique_grasp_labels) > 8 else ''})"
        )

        pre_place_successes = _evaluate_curobo_pose_candidates_multi_start(
            planner,
            demo,
            args,
            pair_candidates,
            label="joint_direct_place_pairs" if use_direct_place_pairs else "joint_pre_place_pairs",
            use_attach=True,
            ...
            max_winners=goalset_max_winners,
        )
        if not pre_place_successes:
            print("[joint_search] no grasp/place pair passed cuRobo")
            return []

        pre_place_successes.sort(
            key=lambda item: (
                float(item["grasp_choice"]["score"]) + float(item["score"]),
                float(item["score"]),
                float(item["grasp_choice"]["score"]),
            )
        )
        for candidate in pre_place_successes:
            ...
            if use_direct_place_pairs:
                q_place_candidate = _plan_place_motion_from_pre_place(
                    planner,
                    demo,
                    args,
                    ...
                    label=f"final_place_{candidate['label']}",
                ...
                print(
                    f"[joint_search] feasible direct-place chain: grasp={grasp_choice['label']}, "
                    f"place={candidate['label']}, total_score={total_score:.3f}, "
                    f"final_place_waypoints={release_metrics['waypoint_count']}"
                )
                chains.append(chain)
            ...
            q_place_candidate = _plan_place_motion_from_pre_place(
                ...
                candidate["pose"],
                candidate["place_pose"],
                label=f"release_{candidate['label']}",
                primitive=rule.primitive,
            )
            if q_place_candidate is None:
                continue
            ...
            print(
                f"[joint_search] feasible chain: grasp={grasp_choice['label']}, "
...
def run_targeted_place_episode_curobo_direct(
    ...
    grasp_successes = _evaluate_curobo_pose_candidates_goalset(
        ...
        prefer_verticality=False,
        use_attach=False,
        max_winners=int(getattr(args, "direct_grasp_goalset_max_winners", 0)),
        include_active_object=False,
    )
    if not grasp_successes:
        ...
        return False
    print(
        f"[direct_grasp] successful winner count: {len(grasp_successes)} "
        f"(labels={[str(item['label']) for item in grasp_successes[:8]]}{'...' if len(grasp_successes) > 8 else ''})"
    )
