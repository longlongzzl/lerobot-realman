...
class RealManArm(Robot):
    ...
    def _udp_gripper_to_obs(self, raw: float) -> float:
        ...
        g = max(0.0, min(9000.0, float(raw)))
        return (g / 9000.0) * 0.91
    ...
    def send_cmd_nr(self, cmd_6axis: str) -> int:
        if not self.client:
            raise RuntimeError("socket 尚未建立")
        self.client.send(cmd_6axis.encode("utf-8"))
        return 0

    def reset(self) -> None:
        self.arm.rm_set_hand_follow_pos([0], False)
        self.arm.rm_movej([90, 0, 0, -90, 0, -90, 60], 100, 0, 0, 1)
        # self.arm.rm_movej([21.9901, -45.6991, -43.9000, -119.0320, 51.6464, -127.5404, -123.3521], 20, 0, 0, 1)
        self.arm.rm_set_arm_max_line_speed(0.0001)
        self.arm.rm_set_hand_speed(1000)
        ip = '192.168.101.20'
    ...
    def get_observation(self) -> Dict[str, Any]:
        ...
        if self.config.enable_gripper:
            try:
                ...
                obs["gripper.pos"] = float(self._get_gripper_modbus())* 0.91
                # else:
    ...
    def _get_current_joint_degrees(self) -> List[float]:
        """
        Prefer从UDP缓存读取当前关节角度，缓存不可用时退回SDK调用。
        Always返回单位为度的列表。
        """
        self._require_connected()
        assert self.arm is not None
        ...
        if cached is not None:
            try:
                cur_exposed = [float(cached[f"{jn}.pos"]) for jn in self._joint_names]
                return self._list_exposed_to_deg(cur_exposed)
            except KeyError:
                pass

        code, cur_deg = self.arm.rm_get_joint_degree()
        self._check_ok(code, "rm_get_joint_degree (current joints)")
        cur_deg_list = list(cur_deg)
        if len(cur_deg_list) != self._dof:
            self._dof = len(cur_deg_list)
            if len(self._joint_names) != self._dof:
                self._joint_names = [f"joint{i + 1}" for i in range(self._dof)]
        return cur_deg_list
    ...
    def send_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if provided_joint:
            ...
            cur_deg_list = self._get_current_joint_degrees()
            ...
            # print("goal_deg", goal_deg)
            if self.client:
                # cmd = {
                #     "command": "movej_canfd",
                #     "joint": [int(x*1000) for x in goal_deg],
                #     "follow": False,
                #     "expand": 0,
                #     "trajectory_mode": 1,
                #     "radio": 0,
                ...
                move_cmd = json.dumps(cmd) + "\r\n"
                rc = self.send_cmd_nr(cmd_6axis=move_cmd)
            else:
                # Call movej: rm_movej(joint, v, r, connect, block) -> int
                rc = self.arm.rm_movej(goal_deg, 90, 90, 0, True)
                # print("goal_deg", goal_deg)
                # rc = self.arm.rm_movej_canfd(goal_deg, True, 0, 0, 10)
        ...
        if self.config.enable_gripper and ("gripper.pos" in action):
            ...
            g = float(action["gripper.pos"]) / 0.91  # 为0-1处理的函数 但是这里要放大一波 实际上传入的是0-0.91
            # delta_norm = g / 0.91
            ...
            if self.config.gripper_normed:
                ...
                # self.arm.rm_set_hand_follow_pos([int(g*9000)], True)
                g_int = int(max(0.0, min(1.0, g)) * 9000)
                cmd = {"command": "hand_follow_pos", "hand_pos": [g_int]}
                point6_00 = json.dumps(cmd) + "\r\n"
                _ = self.send_cmd_nr(cmd_6axis=point6_00)
