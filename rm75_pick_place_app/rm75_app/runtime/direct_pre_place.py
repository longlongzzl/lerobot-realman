...
from rm75_app.runtime import targeted_place as targeted
from rm75_app.runtime import targeted_curobo as curobo_wrapper
from rm75_app.perception import wrist_relation
from rm75_app.paths import DEFAULT_CUROBO_CFG, RUNTIME_DIR
...
def _profile_auto_path(args) -> Path:
    ...
    if raw_jsonl:
        ...
        profile_dir = Path(
            str(getattr(args, "planning_profile_dir", RUNTIME_DIR / "planning_profile_logs") or (RUNTIME_DIR / "planning_profile_logs"))
        ).expanduser()
...
def _record_prefetch_profile(base_args, object_name, stage_name: str, **fields) -> None:
    ...
    profile_args = SimpleNamespace(
        ...
        planning_profile_dir=getattr(base_args, "planning_profile_dir", str(RUNTIME_DIR / "planning_profile_logs")),
    )
...
def build_arg_parser():
    ...
    parser.set_defaults(
        ...
        curobo_rm75_robot_cfg=DEFAULT_CUROBO_CFG,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Install one or more runtime targeted-place rules from JSON before planning. Used by the LLM orchestrator.",
    )
    parser.add_argument(
        "--allow-repeated-cycle-object-names",
        action="store_true",
        default=False,
        help="Accepted for LLM-generated multi-step commands; duplicate cycle names are handled in sequence when present.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=str(RUNTIME_DIR / "planning_profile_logs"),
    ...
    parser.add_argument(
        ...
        dest="wrist_relation_refine_after_grasp",
        action="store_true",
        default=False,
        help=(
            "After the gripper closes, run wrist-camera RGB-D refinement for the held object and update "
            "the TCP-object relation before targeted place planning. Disabled by default so existing batch "
            "flows are unchanged."
        ),
    )
    parser.add_argument(
        "--no-wrist-relation-refine-after-grasp",
        dest="wrist_relation_refine_after_grasp",
        action="store_false",
        help="Disable wrist-camera held-object relation refinement.",
    )
    parser.add_argument(
        "--wrist-relation-script",
        type=Path,
        default=wrist_relation.DEFAULT_WRIST_SCRIPT,
        help="Standalone wrist relation estimator script used by the app adapter.",
    )
    parser.add_argument(
        "--wrist-relation-python",
        type=str,
        default=sys.executable,
        help="Python executable used to launch the wrist relation estimator.",
    )
    parser.add_argument(
        "--wrist-relation-hand-eye-path",
        type=Path,
        default=None,
        help="Path to ee_T_wrist_camera.npy/json. If omitted, the latest calibration run is used.",
    )
    parser.add_argument(
        "--wrist-relation-realsense-serial",
        type=str,
        default="",
        help="Wrist RealSense serial. Empty lets the estimator choose the first available RealSense.",
    )
    parser.add_argument(
        "--wrist-relation-prompt",
        type=str,
        default=None,
        help="SAM3 prompt override for the held object in the wrist image.",
    )
    parser.add_argument(
        "--wrist-relation-output-dir",
        type=Path,
        default=RUNTIME_DIR / "wrist_relation_runs",
        help="Directory for per-run wrist relation debug outputs.",
    )
    parser.add_argument(
        "--wrist-relation-latest-path",
        type=Path,
        default=RUNTIME_DIR / "wrist_relation_latest.json",
        help="Latest wrist relation JSON path written by the estimator.",
    )
    parser.add_argument(
        "--wrist-relation-timeout-s",
        type=float,
        default=45.0,
        help="Hard timeout for one wrist relation refinement subprocess.",
    )
    parser.add_argument(
        ...
        default=3.0,
        help=(
            "Maximum time to wait for the async wrist refinement right before building place candidates. "
            "Set <=0 to use the result only if it has already finished."
        ),
    )
    parser.add_argument(
        "--wrist-relation-apply-mode",
        choices=["before_place_planning", "during_transport"],
        default="before_place_planning",
        help=(
            "before_place_planning keeps the legacy behavior. during_transport starts refinement after grasp, "
            "executes the original transport hover while it runs, then applies the relation at hover and plans "
            "a precise pre_place_new/final-contact segment."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.75,
        help="Extra wait budget at the original pre-place hover for a during-transport wrist relation result.",
    )
    parser.add_argument(
        "--wrist-relation-refined-place-max-candidates",
        type=int,
        default=12,
        help="Limit hover candidates when replanning the precise pre_place_new segment after a wrist update.",
    )
    parser.add_argument(
        "--wrist-relation-require-refined-place",
        dest="wrist_relation_require_refined_place",
        action="store_true",
        default=False,
        help="If a wrist relation is applied at pre-place but no refined insertion plan is found, fail instead of using the old plan.",
    )
    parser.add_argument(
        "--no-wrist-relation-require-refined-place",
        dest="wrist_relation_require_refined_place",
        action="store_false",
        help="Fallback to the original place plan if refined pre_place_new planning fails.",
    )
    parser.add_argument(
        "--wrist-relation-min-confidence",
        type=float,
        default=0.25,
        help="Minimum wrist estimator confidence required before updating the held-object relation.",
    )
    parser.add_argument(
        "--wrist-relation-min-iou",
        type=float,
        default=0.08,
        help="Minimum wrist estimator projection IoU required before updating the held-object relation.",
    )
    parser.add_argument(
        "--wrist-relation-require-held",
        dest="wrist_relation_require_held",
        action="store_true",
        default=True,
        help="Require the wrist estimator's grasp-state classifier to report held/likely_held.",
    )
    parser.add_argument(
        "--no-wrist-relation-require-held",
        dest="wrist_relation_require_held",
        action="store_false",
        help="Trust an accepted wrist relation even if grasp-state is not held.",
    )
    parser.add_argument(
        "--wrist-relation-max-delta-m",
        type=float,
        default=0.080,
        help="Reject wrist updates whose TCP-object translation differs from the planned grasp relation by more than this.",
    )
    parser.add_argument(
        "--wrist-relation-max-rot-delta-deg",
        type=float,
        default=90.0,
        help="Reject wrist updates whose TCP-object rotation differs from the planned grasp relation by more than this.",
    )
    parser.add_argument(
        "--wrist-relation-extra-arg",
        action="append",
        default=[],
        help="Extra argument passed through to wrist_gripper_object_relation.py. Repeat for multiple args.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="insert_place_partial_open_at_release",
        action="store_true",
        default=False,
        help=(
            "For insert_place, only open the gripper slightly at the insertion pose, "
            "keep that partial opening during post-place clearance, then fully open after clearance."
        ),
    )
    parser.add_argument(
        "--no-insert-place-partial-open-at-release",
        dest="insert_place_partial_open_at_release",
        action="store_false",
        help="Disable the two-stage insert_place release opening.",
    )
    parser.add_argument(
        ...
        default=0.04,
        help=(
            "Absolute real-gripper command delta used for the first insert_place release. "
            "With the default RM75 convention, lower values are more open, so the command is "
            "current_gripper_pos - delta when a current reading is available."
        ),
    )
    parser.add_argument(
        "--insert-place-partial-open-sim-steps",
        type=int,
        default=2,
        help="Simulation preview steps for the first partial insert_place release opening.",
    )
    parser.add_argument(
        "--insert-place-full-open-after-clearance",
        dest="insert_place_full_open_after_clearance",
        action="store_true",
        default=True,
        help="After an insert_place partial release, fully open the gripper once the empty gripper has retreated.",
    )
    parser.add_argument(
        "--no-insert-place-full-open-after-clearance",
        dest="insert_place_full_open_after_clearance",
        action="store_false",
        help="Keep the partial opening after insert_place clearance instead of fully opening.",
    )
    parser.add_argument(
...
def parse_args():
    ...
    if getattr(args, "dynamic_place_rule_json", None):
        installed = targeted.install_dynamic_place_rules_from_file(args.dynamic_place_rule_json)
        if installed:
            print(f"[dynamic_place_rule] installed runtime rules: {installed}")
...
def _insert_place_partial_open_command(args, current_gripper_pos: float | None = None) -> float:
    open_cmd = float(getattr(args, "real_gripper_open", 0.0))
    close_cmd = float(getattr(args, "real_gripper_close", 0.91))
    delta = float(max(getattr(args, "insert_place_partial_open_delta", 0.04), 0.0))
    lo = min(open_cmd, close_cmd)
    hi = max(open_cmd, close_cmd)
    base = close_cmd
    if current_gripper_pos is not None:
        try:
            current = float(current_gripper_pos)
            if np.isfinite(current):
                base = current
        except Exception:
            pass
    if close_cmd >= open_cmd:
        cmd = base - delta
    else:
        cmd = base + delta
...
def _real_gripper_command_to_sim_value(args, real_gripper_pos: float) -> float:
    real_open = float(getattr(args, "real_gripper_open", 0.0))
    real_close = float(getattr(args, "real_gripper_close", 0.91))
    sim_open = float(getattr(args, "gripper_open", -1.0))
    sim_close = float(getattr(args, "gripper_close", 1.0))
    denom = real_close - real_open
    if abs(denom) <= 1e-6:
        return sim_close
    close_fraction = (float(real_gripper_pos) - real_open) / denom
    close_fraction = float(np.clip(close_fraction, 0.0, 1.0))
    return float(sim_open + close_fraction * (sim_close - sim_open))


def _sync_demo_gripper_to_real_command(
    demo,
    args,
    real_gripper_pos: float,
    *,
    steps: int = 3,
    label: str = "partial opened",
) -> None:
    sim_gripper_value = _real_gripper_command_to_sim_value(args, real_gripper_pos)
    ...
    try:
        ...
        targeted.base.sync_planner_qpos_from_demo(demo)
        targeted.base.update_attached_box_visual(demo)
        gap = targeted.base.get_sim_gripper_pad_gap(demo)
        if gap is not None:
            print(
                f"[sim gripper] {label} preview after {total_steps} steps, "
                f"real_cmd={float(real_gripper_pos):.3f}, sim_cmd={sim_gripper_value:.3f}, "
                f"pad gap={gap:.4f} m"
            )
    except Exception as exc:
        print(f"[warn] failed to sync simulated gripper partial command: {exc}")
        try:
            if os.environ.get("RM75_DEBUG_GRIPPER_SYNC_TRACE"):
                traceback.print_exc()
        except Exception:
            pass
...
def _execute_fused_place_hover_release_with_async_empty_check(
    ...
    with _profile_stage(
        ...
        dense_ok = targeted.base.validate_joint_path_segments(
            demo,
            q_start,
            fused_path,
            use_attach=True,
            label=label,
            max_delta=targeted.base._dense_collision_validate_max_delta(True),
            allow_start_in_collision=False,
        )
        if not dense_ok:
            prof["success"] = False
            prof["status"] = "DENSE_VALIDATE_FAIL"
            return False, None, check_result
...
def _rotation_delta_deg_from_matrices(T_a: np.ndarray, T_b: np.ndarray) -> float:
    R = np.asarray(T_a, dtype=np.float64).reshape(4, 4)[:3, :3] @ np.asarray(T_b, dtype=np.float64).reshape(4, 4)[:3, :3].T
    value = float(np.clip((np.trace(R) - 1.0) * 0.5, -1.0, 1.0))
    return float(np.degrees(np.arccos(value)))


def _flatten_wrist_relation_extra_args(args) -> tuple[str, ...]:
    import shlex

    values = []
    for item in list(getattr(args, "wrist_relation_extra_arg", []) or []):
        values.extend(shlex.split(str(item)))
    return tuple(values)


def _start_wrist_relation_after_grasp(demo, args) -> wrist_relation.WristRelationJob | None:
    if not bool(getattr(args, "wrist_relation_refine_after_grasp", False)):
        return None
    T_tcp_obj = getattr(demo, "_transport_attached_T_tcp_obj", None)
    if T_tcp_obj is None:
        print("[wrist_relation] skipped: no planned T_tcp_obj prior is available after grasp")
        return None
    try:
        prior_dir = RUNTIME_DIR / "wrist_relation_priors"
        prior_path = prior_dir / f"{int(time.time_ns())}_{getattr(args, 'object_name', 'object')}_T_gripper_obj.json"
        wrist_relation.write_matrix_json(prior_path, "T_gripper_obj", np.asarray(T_tcp_obj, dtype=np.float64).reshape(4, 4))
        config = wrist_relation.WristRelationConfig(
            object_name=str(getattr(args, "object_name", "")),
            script_path=Path(getattr(args, "wrist_relation_script", wrist_relation.DEFAULT_WRIST_SCRIPT)),
            python=str(getattr(args, "wrist_relation_python", sys.executable)),
            output_dir=Path(getattr(args, "wrist_relation_output_dir", RUNTIME_DIR / "wrist_relation_runs")),
            relation_out=Path(getattr(args, "wrist_relation_latest_path", RUNTIME_DIR / "wrist_relation_latest.json")),
            hand_eye_path=getattr(args, "wrist_relation_hand_eye_path", None),
            init_T_gripper_obj_path=prior_path,
            realsense_serial=str(getattr(args, "wrist_relation_realsense_serial", "") or ""),
            prompt=getattr(args, "wrist_relation_prompt", None),
            timeout_s=float(getattr(args, "wrist_relation_timeout_s", 45.0)),
            min_confidence=float(getattr(args, "wrist_relation_min_confidence", 0.25)),
            min_projection_iou=float(getattr(args, "wrist_relation_min_iou", 0.08)),
            require_held=bool(getattr(args, "wrist_relation_require_held", True)),
            extra_args=_flatten_wrist_relation_extra_args(args),
        )
        _record_profile(
            args,
            "wrist_relation_start",
            success=True,
            status="STARTED",
            object_name=str(getattr(args, "object_name", "")),
            prior_path=str(prior_path),
            relation_out=str(config.relation_out),
        )
        print(f"[wrist_relation] started async held-object refinement for {config.object_name}")
        return wrist_relation.start_async(config)
    except Exception as exc:
        _record_profile(
            args,
            "wrist_relation_start",
        ...
        print(f"[wrist_relation] failed to start: {exc}")
        return None


def _wait_and_apply_wrist_relation(
    demo,
    args,
    planner,
    job: wrist_relation.WristRelationJob | None,
    *,
    label: str,
    ...
    if job is None:
        return False
    timeout_s = (
        float(getattr(args, "wrist_relation_wait_before_place_s", 3.0))
        if timeout_s is None
        else float(timeout_s)
    )
    if timeout_s <= 0.0 and not job.done():
        print(f"[wrist_relation] {label}: result not ready; continuing with planned TCP-object relation")
        _record_profile(args, "wrist_relation_apply", success=False, status="NOT_READY", phase=label)
        return False
    try:
        result = job.wait(timeout_s=max(0.0, timeout_s))
    except Exception as exc:
        print(f"[wrist_relation] {label}: estimator failed: {exc}")
        _record_profile(
            args,
            "wrist_relation_apply",
            success=False,
            status=type(exc).__name__,
            phase=label,
            error=str(exc),
    ...
    if result is None:
        print(
            f"[wrist_relation] {label}: estimator still running after {timeout_s:.2f}s; "
            "continuing with planned TCP-object relation"
        )
        _record_profile(args, "wrist_relation_apply", success=False, status="TIMEOUT_WAITING", phase=label)
        return False
    if not result.accepted or result.T_gripper_obj is None:
        print(
            f"[wrist_relation] {label}: rejected relation "
            f"reason={result.reason} conf={result.confidence:.3f} iou={result.projection_iou:.3f} "
            f"grasp={result.grasp_state}"
        )
        _record_profile(
            args,
            "wrist_relation_apply",
            ...
            projection_iou=float(result.projection_iou),
            grasp_state=result.grasp_state,
        )
        return False

    T_new = np.asarray(result.T_gripper_obj, dtype=np.float32).reshape(4, 4)
    T_old = getattr(demo, "_transport_attached_T_tcp_obj", None)
    trans_delta = None
    rot_delta = None
    if T_old is not None:
        T_old = np.asarray(T_old, dtype=np.float32).reshape(4, 4)
        trans_delta = float(np.linalg.norm(T_new[:3, 3] - T_old[:3, 3]))
        rot_delta = _rotation_delta_deg_from_matrices(T_new, T_old)
        if trans_delta > float(getattr(args, "wrist_relation_max_delta_m", 0.080)):
            print(f"[wrist_relation] {label}: rejected large translation delta {trans_delta:.4f} m")
            _record_profile(args, "wrist_relation_apply", success=False, status="TRANSLATION_DELTA_REJECT", phase=label, translation_delta_m=trans_delta, rotation_delta_deg=rot_delta)
            return False
        if rot_delta > float(getattr(args, "wrist_relation_max_rot_delta_deg", 90.0)):
            print(f"[wrist_relation] {label}: rejected large rotation delta {rot_delta:.2f} deg")
            _record_profile(args, "wrist_relation_apply", success=False, status="ROTATION_DELTA_REJECT", phase=label, translation_delta_m=trans_delta, rotation_delta_deg=rot_delta)
            return False

    demo._transport_attached_T_tcp_obj = T_new
    ...
    targeted._register_transport_attached_box(
        ...
        show_visual=False,
        activate_payload_visual=False,
        T_tcp_obj_override=T_new,
    )
    if bool(getattr(args, "curobo_attach_object", True)):
        _attach_transport_payload_to_curobo(planner, demo, args, label=f"{label}_wrist_relation")
    print(
        f"[wrist_relation] applied refined T_tcp_obj from {result.relation_path} "
        f"delta_m={0.0 if trans_delta is None else trans_delta:.4f} "
        f"delta_deg={0.0 if rot_delta is None else rot_delta:.2f} "
        f"forced_active_object={forced}"
    )
    _record_profile(
        args,
        "wrist_relation_apply",
        ...
        status="APPLIED",
        phase=label,
        relation_path=str(result.relation_path),
        confidence=float(result.confidence),
        projection_iou=float(result.projection_iou),
        grasp_state=result.grasp_state,
        translation_delta_m=trans_delta,
        rotation_delta_deg=rot_delta,
    )
    return True


def _wrist_relation_during_transport_enabled(args) -> bool:
    return (
        bool(getattr(args, "wrist_relation_refine_after_grasp", False))
        and str(getattr(args, "wrist_relation_apply_mode", "before_place_planning")) == "during_transport"
    )


def _limit_wrist_refined_place_candidates(candidates, args) -> list[dict]:
    items = sorted([dict(item) for item in list(candidates or [])], key=_pre_place_screen_sort_key)
    try:
        limit = int(getattr(args, "wrist_relation_refined_place_max_candidates", 12) or 0)
    except Exception:
        limit = 12
    if limit > 0 and len(items) > limit:
        print(
            f"[wrist_relation] pre_place_new limiting refined hover candidates "
            f"{len(items)}->{limit} for speed"
        )
        return items[:limit]
    return items


def _plan_wrist_refined_place_from_current_hover(
    *,
    planner,
    demo,
    bridge_mod,
    ...
    rule,
    place_state_cache,
    direct_place_disabled_links: list[str] | None,
    exclude_names_place: set[str] | None,
) -> tuple[dict | None, list[np.ndarray], list[np.ndarray]]:
    T_tcp_obj_refined = getattr(demo, "_transport_attached_T_tcp_obj", None)
    if T_tcp_obj_refined is None:
        print("[wrist_relation] pre_place_new skipped: no refined T_tcp_obj on demo")
        _record_profile(args, "wrist_relation_pre_place_new", success=False, status="NO_REFINED_T_TCP_OBJ")
        return None, [], []
    ...
    targeted._register_transport_attached_box(
        demo,
        args,
        T_tcp_obj_override=T_tcp_obj_refined,
    )
    if bool(getattr(args, "curobo_attach_object", True)):
        _attach_transport_payload_to_curobo(planner, demo, args, label="pre_place_new_wrist_relation")

    all_candidates = _build_direct_place_candidates(
        demo,
        bridge_mod,
        scene_capture_cache,
        rule,
    ...
    if rule.primitive == "insert_vertical":
        all_candidates = _filter_pre_place_candidates_by_verticality(all_candidates, args)
    candidates = _limit_wrist_refined_place_candidates(all_candidates, args)
    if not candidates:
        print("[wrist_relation] pre_place_new failed: no refined hover/release candidates")
        _record_profile(
            args,
            "wrist_relation_pre_place_new",
            success=False,
    ...
    candidate_passes: list[tuple[str, list[dict]]] = [("limited", candidates)]
    if len(candidates) < len(all_candidates):
        candidate_passes.append(("full_retry", sorted([dict(item) for item in all_candidates], key=_pre_place_screen_sort_key)))
    transport_successes = []
    selected_pass = "limited"
    for pass_label, pass_candidates in candidate_passes:
        if pass_label != "limited":
            print(
                f"[wrist_relation] pre_place_new retrying with full candidate set "
                f"({len(pass_candidates)} candidate(s)) after limited pass failed"
            )
        ...
        selected_pass = pass_label
        if transport_successes:
            candidates = pass_candidates
            break
    if not transport_successes:
        print("[wrist_relation] pre_place_new failed: no transport path from old hover to refined hover")
        _record_profile(
            args,
            "wrist_relation_pre_place_new",
            success=False,
            status="NO_TRANSPORT_WINNERS",
    ...
    for transport_choice in transport_successes:
        refined_choice = plan_final_contact_approach(
            planner,
            demo,
            args,
            current_q,
        ...
        if refined_choice is not None:
            break
    if refined_choice is None:
        print("[wrist_relation] pre_place_new failed: refined final-contact planning failed")
        _record_profile(
            args,
            "wrist_relation_pre_place_new",
            success=False,
            status="FINAL_CONTACT_FAIL",
            candidate_count=len(candidates),
            full_candidate_count=len(all_candidates),
            selected_pass=selected_pass,
            transport_winner_count=len(transport_successes),
            elapsed_ms=round((time.perf_counter() - t0) * 1000.0, 3),
        )
        return None, [], []
    ...
    q_pre_new = [
        ...
        for q in refined_choice.get("q_pre_place_path", refined_choice["q_path"])
    ]
    q_place_new = [
        np.asarray(q, dtype=np.float32).reshape(-1)[:7]
        for q in refined_choice.get("q_place_path", [np.asarray(q_pre_new[-1], dtype=np.float32).reshape(-1)[:7]])
    ]
    refined_choice["wrist_relation_pre_place_new"] = True
    refined_choice["label"] = f"{refined_choice.get('label', 'place')}_wrist_refined"
    print(
        "[wrist_relation] pre_place_new planned: "
        f"transport_waypoints={len(q_pre_new)}, place_waypoints={len(q_place_new)}, "
    ...
    _record_profile(
        ...
        selected_pass=selected_pass,
        transport_winner_count=len(transport_successes),
        q_pre_place_new_waypoints=len(q_pre_new),
        q_place_waypoints=len(q_place_new),
        selected_label=str(refined_choice.get("label", "")),
        elapsed_ms=round((time.perf_counter() - t0) * 1000.0, 3),
    )
    return refined_choice, q_pre_new, q_place_new
...
def _direct_place_contact_tolerant_disabled_links(planner) -> list[str]:
    # Keep all real gripper links collision-checked against the table/scene.
    # Earlier this relaxed support-link collisions to make contact plans easier,
    # but it can visibly allow the gripper body to pass through the tabletop.
    return []
...
def _candidate_target_up_axis_world(candidate, demo) -> np.ndarray | None:
    ...
    if target_name:
        ...
        if scene_entry is not None and scene_entry.get("T_world_obj") is not None:
            ...
            if rule is not None:
                return targeted._target_place_up_axis(rule, T_world_target)
...
def _tilt_topdown_pose_along_tcp_x(pose, tilt_deg: float):
    """Tilt the top-down approach toward TCP +/-X while keeping TCP +Y fixed.

    For long-axis parts we build TCP +X along the object long axis and TCP +Y
    across the object.  Rotating locally about TCP +Y makes the approach vector
    lean along the part length, which is the physical "long-axis tilt" needed
    for tilted insertion-style grasps.
    """
    try:
        tilt_deg = float(tilt_deg)
        if abs(tilt_deg) <= 1e-6:
            return pose
        T_world_tcp = _pose_to_matrix_from_pose_obj(pose)
        R_tcp = T_world_tcp[:3, :3].astype(np.float32)
        ...
        T_world_tcp[:3, :3] = (R_tcp @ R_tilt_local_y).astype(np.float32)
        return _pose_from_world_matrix(T_world_tcp)
    except Exception:
        return None


def _build_tingzi_pillar_pose_variants(args, raw_grasp_pose) -> list[tuple[str, object]]:
    ...
    # The pillar is axial-symmetric for placement.  Do not add yaw variants to
    # the grasp set: the fixed 16-slot grasp batch should be spent on long-axis
    # tilt/clearance variants, while 0/90/180/270 spin is evaluated separately at
    # the insertion stage.
    configured_tilts = _unique_finite_float_list(
        getattr(
            args,
            "direct_elongated_grasp_tilt_toward_robot_deg",
            getattr(args, "direct_grasp_tilt_toward_robot_deg", [12.0, 20.0]),
        )
    )
    if not any(float(v) < -1e-6 for v in configured_tilts):
        nonzero = [abs(float(v)) for v in configured_tilts if abs(float(v)) > 1e-6]
        if len(nonzero) <= 2:
            configured_tilts = [
                0.0,
                6.0,
    ...
    explicit_signed = any(float(v) < -1e-6 for v in configured_tilts)

    signed_tilts: list[float] = []
    if explicit_signed:
        signed_tilts = [float(v) for v in configured_tilts]
    else:
        signed_tilts.append(0.0)
        for tilt_deg in configured_tilts:
            mag = abs(float(tilt_deg))
            if mag <= 1e-6:
                continue
            signed_tilts.extend([mag, -mag])

    for signed_tilt in signed_tilts:
        signed_tilt = float(signed_tilt)
        if abs(signed_tilt) <= 1e-6:
            _append("grasp_long_axis_side", raw_grasp_pose)
            continue
        sign_label = "pos" if signed_tilt > 0.0 else "neg"
        tilted_pose = _tilt_topdown_pose_along_tcp_x(raw_grasp_pose, signed_tilt)
        _append(
            f"grasp_tilt_along_long_{sign_label}_{int(round(abs(signed_tilt)))}deg",
            tilted_pose,
        )
    return variants


def _filter_tingzi_pillar_grasp_candidates(candidates: list[dict], object_axis_world: np.ndarray | None) -> list[dict]:
    long_axis = _normalize(object_axis_world)
    if long_axis is None or not candidates:
        return candidates
    # TCP +Y is the pad-opening/closing axis.  If it is parallel to the
    # pillar's long axis, the gripper is pinching along the pillar length
    # instead of across it.
    max_parallel_dot = 0.55
    ordered = []
    deprioritized = []
    for idx, item in enumerate(list(candidates or [])):
        pose = item.get("pose")
        parallel_dot = 1.0
        try:
            R_world_tcp = _pose_to_matrix_from_pose_obj(pose)[:3, :3].astype(np.float32)
            pad_open_axis = _normalize(R_world_tcp[:, 1])
            if pad_open_axis is None:
                ordered.append((1, 1.0, idx, item))
                continue
            parallel_dot = abs(float(np.dot(pad_open_axis, long_axis)))
            item["grasp_pad_axis_long_axis_dot"] = parallel_dot
        except Exception:
            ordered.append((1, 1.0, idx, item))
            continue
        if parallel_dot <= max_parallel_dot + 1e-6:
            ordered.append((0, parallel_dot, idx, item))
        else:
            ordered.append((1, parallel_dot, idx, item))
            deprioritized.append((str(item.get("label", "?")), parallel_dot))
    if deprioritized:
        print(
            f"[direct_grasp] tingzi pillar side-grasp order: "
            f"deprioritized {len(deprioritized)}/{len(candidates)} pad-axis-parallel grasp(s): "
            + ", ".join(f"{label}:{dot:.2f}" for label, dot in deprioritized[:8])
        )
    ordered.sort(key=lambda x: (x[0], x[1], x[2]))
    return [item for _, _, _, item in ordered] or candidates
...
def _build_direct_grasp_candidates(
    ...
    *,
    ...
    if _is_tingzi_source(source_name):
        yaw_values = _unique_finite_float_list(
            getattr(args, "topdown_grasp_yaw_variant_deg", [0.0, 90.0, 180.0, 270.0])
        )
        grasp_variant_args.topdown_grasp_yaw_variant_deg = yaw_values or [0.0, 90.0, 180.0, 270.0]
    else:
    ...
    elif _is_tingzi_pillar_source(source_name):
        pillar_long_axis_world = None
        try:
            pillar_long_axis_world = _fixed_tabletop_horizontal_long_axis(
                args,
                T_world_obj0,
                object_dims=object_extents,
                fallback_axis=object_axis_world,
            ...
            pillar_long_axis_world = None
        if pillar_long_axis_world is None and object_axis_world is not None:
            pillar_long_axis_world = object_axis_world
        pillar_grasp_pose = _make_long_axis_perpendicular_topdown_grasp_pose(
            raw_grasp_pose,
            pillar_long_axis_world,
            label=str(source_name or "tingzi_pillar"),
        )
        if pillar_grasp_pose is not None:
            raw_grasp_pose = pillar_grasp_pose
            object_axis_world = pillar_long_axis_world
    ...
    def _build_pose_variants():
        ...
        if use_rule_bias_variants:
            ...
            if _is_tingzi_pillar_source(source_name):
                for label, pose in _build_tingzi_pillar_pose_variants(args, raw_grasp_pose):
                    _append(label, pose)
            else:
        ...
        if use_rule_bias_variants:
            ...
            tilt_degs = []
        elif grasp_mode in elongated_modes:
            tilt_degs = _unique_finite_float_list(
                getattr(
                    args,
                    "direct_elongated_grasp_tilt_toward_robot_deg",
                    getattr(args, "direct_grasp_tilt_toward_robot_deg", [12.0, 20.0, 30.0, 45.0]),
                )
            )
        else:
        ...
        if use_rule_bias_variants:
            ...
            shift_ds = _unique_finite_float_list(
                getattr(
                    args,
                    "direct_elongated_grasp_tilt_toward_robot_shift_m",
    ...
    if fixed_tabletop_place_first_candidates:
        ...
        candidates = merged
    if _is_tingzi_pillar_source(source_name):
        candidates = _filter_tingzi_pillar_grasp_candidates(candidates, object_axis_world)
...
def _is_tingzi_source(source_name: str | None) -> bool:
    return str(source_name or "").strip().lower() in TINGZI_SOURCE_NAMES


def _is_tingzi_pillar_source(source_name: str | None) -> bool:
    return str(source_name or "").strip().lower().startswith("tingzi_pillar_")
...
def _insert_vertical_target_axis_world(rule, T_world_target: np.ndarray) -> np.ndarray | None:
    axis_local = getattr(rule, "insert_axis_local", None)
    if axis_local is None:
        axis_local = (0.0, 1.0, 0.0)
    axis_local = np.asarray(axis_local, dtype=np.float32).reshape(3)
    axis_local_norm = _normalize(axis_local)
    if axis_local_norm is None:
        axis_local_norm = np.array([0.0, 1.0, 0.0], dtype=np.float32)
    target_axis = _normalize(np.asarray(T_world_target[:3, :3], dtype=np.float32).reshape(3, 3) @ axis_local_norm)
    if target_axis is None:
        return None
    pose_spec = getattr(rule, "object_pose_local", None)
    sign = 1.0
    if pose_spec is not None:
        try:
            local_position = np.asarray(getattr(pose_spec, "position", (0.0, 1.0, 0.0)), dtype=np.float32).reshape(3)
            if float(np.dot(local_position, axis_local_norm)) < 0.0:
                sign = -1.0
        except Exception:
            sign = 1.0
    return (target_axis * sign).astype(np.float32)
...
def classify_place_mode(args, rule, object_dims=None) -> str:
    ...
    if str(getattr(rule, "primitive", "")) == "roof_assembly":
        return "surface_place"
...
def build_hover_pose(release_pose, place_mode: str, args, rule, *, candidate_pre_place_pose=None):
    ...
    if str(getattr(rule, "primitive", "")) == "roof_assembly" and candidate_pre_place_pose is not None:
        return candidate_pre_place_pose
...
def _build_direct_pre_place_candidates(demo, bridge_mod, scene_capture_cache, rule, place_state_cache, args, *, T_tcp_obj_override=None):
    ...
    if rule.primitive == "insert_vertical":
        ...
        if T_world_target is not None:
            target_axis = _insert_vertical_target_axis_world(rule, T_world_target)
...
def _raw_grasp_relation_sort_key(item, args, rule, source_name: str | None) -> tuple:
    ...
    if _is_tingzi_pillar_source(source):
        tilt = float(_fixed_tabletop_signed_tilt_from_label(label))
        return (
            0 if abs(tilt) <= 1.0 else 1,
            abs(tilt),
            0 if tilt >= 0.0 else 1,
            axis_shift,
            z_lift,
            label,
        )
...
def _tingzi_pillar_fast_chain_pair_sort_key(item: dict) -> tuple:
    grasp = item.get("grasp_candidate") if isinstance(item, dict) else None
    grasp_label = str((grasp or {}).get("label", "") or "").lower()
    signed_tilt = float(_fixed_tabletop_signed_tilt_from_label(grasp_label))
    tilt_abs = abs(signed_tilt)
    # For pavilion pillars the straight long-axis side grasp has been the
    # stable insertion solution.  Tilted grasps are kept as fallbacks, but they
    # often pass cheap IK and then fail the real final-contact descent.
    return (
        0 if tilt_abs <= 1.0 and "tilt" not in grasp_label else 1,
        tilt_abs,
        0 if signed_tilt >= 0.0 else 1,
        float(item.get("score", 0.0) or 0.0),
        grasp_label,
    )
...
def _fast_chain_relation_match_key(item: dict | None, source_name: str | None) -> tuple:
    ...
    if _is_tingzi_pillar_source(source):
        tilt = float(_fixed_tabletop_signed_tilt_from_label(label))
        return ("tingzi_pillar_tilt_abs", round(abs(tilt), 3))
    if _is_fixed_tabletop_source(source):
        tilt = float(_fixed_tabletop_signed_tilt_from_label(label))
        if source == "lvmukuai":
...
def _fast_chain_relation_match_text(key: tuple) -> str:
    try:
        if len(key) >= 2 and key[0] == "tingzi_pillar_tilt_abs":
            tilt = float(key[1])
            if abs(tilt) <= 1.0:
                return "0deg"
...
def _attached_release_pose_error(place_candidate: dict | None, grasp_candidate: dict | None) -> tuple[float | None, float | None]:
    if not isinstance(place_candidate, dict) or not isinstance(grasp_candidate, dict):
        return None, None
    release_pose = place_candidate.get("release_pose", place_candidate.get("place_pose", place_candidate.get("pose")))
    T_tcp_obj = grasp_candidate.get("T_tcp_obj")
    expected_T = place_candidate.get("T_world_obj_desired")
    if expected_T is None:
        expected_T = getattr(place_candidate.get("place_plan", None), "T_world_obj_desired", None)
    if release_pose is None or T_tcp_obj is None or expected_T is None:
        return None, None
    try:
        T_world_tcp = _pose_to_matrix_from_pose_obj(release_pose)
        actual_T = T_world_tcp @ np.asarray(T_tcp_obj, dtype=np.float32).reshape(4, 4)
        expected_T = np.asarray(expected_T, dtype=np.float32).reshape(4, 4)
        delta_m = float(np.linalg.norm(actual_T[:3, 3] - expected_T[:3, 3]))
        actual_q = targeted.base.bridge_mod_mat2quat(actual_T[:3, :3]).astype(np.float32)
        expected_q = targeted.base.bridge_mod_mat2quat(expected_T[:3, :3]).astype(np.float32)
        delta_deg = float(np.degrees(_quat_angle_rad_wxyz(actual_q, expected_q)))
        return delta_m, delta_deg
    except Exception:
        return None, None


def _filter_attached_release_consistent_records(records, source_name: str | None, *, label: str) -> list[dict]:
    if not _is_tingzi_pillar_source(source_name):
    ...
    for rec in list(records or []):
        place_candidate = rec.get("place_candidate") if isinstance(rec, dict) else None
        grasp_candidate = rec.get("grasp_candidate") if isinstance(rec, dict) else None
        delta_m, delta_deg = _attached_release_pose_error(place_candidate, grasp_candidate)
        if delta_m is None or delta_deg is None:
            kept.append(rec)
            continue
        worst_m = max(worst_m, float(delta_m))
        worst_deg = max(worst_deg, float(delta_deg))
        if float(delta_m) > 0.005 or float(delta_deg) > 3.0:
            rejected += 1
            if rejected <= 8:
                relation_token = _fast_chain_relation_summary_token(grasp_candidate)
                suffix = str(rec.get("relation_token_suffix", "") or "")
                if suffix:
                    relation_token = f"{relation_token}@{suffix}"
                print(
                    f"[winner_chain] rejected inconsistent attached release {source_name or '?'} "
                    f"{relation_token}: actual-vs-target delta {delta_m:.4f} m, {delta_deg:.1f} deg"
                )
            continue
        kept.append(rec)
    if rejected:
        print(
            f"[winner_chain] {source_name or '?'} {label}: rejected {rejected}/{len(list(records or []))} "
            f"inconsistent attached release record(s), worst={worst_m:.4f} m/{worst_deg:.1f} deg"
        )
    return kept
...
def _fast_chain_insert_spin_values(args, source_name: str | None) -> list[float]:
    source = str(source_name or "").strip().lower()
    if not _is_tingzi_pillar_source(source):
        return []
    values = _unique_finite_float_list(getattr(args, "insert_vertical_axial_spin_deg", [0.0, 90.0, 180.0, 270.0]))
    if not values:
        values = [0.0, 90.0, 180.0, 270.0]
    out: list[float] = []
    seen: set[int] = set()
    for value in values:
        spin = _fast_chain_normalize_yaw_deg(float(value))
        bucket = int(round(spin))
        if bucket in seen:
            continue
        seen.add(bucket)
        out.append(spin)
...
def _fast_chain_insert_spin_token(item: dict, source_name: str | None) -> str:
    spin = _fast_chain_normalize_yaw_deg(_variant_yaw_deg_from_labels(item.get("variant_label"), item.get("label")))
    if abs(spin) <= 1e-6:
        return "spin_0deg"
    return f"spin_{spin:+.0f}deg"


def _fast_chain_select_insert_spin_place_candidates(
    place_candidates,
    args,
    source_name: str | None,
) -> list[dict]:
    target_spins = _fast_chain_insert_spin_values(args, source_name)
    if not target_spins:
        return []
    ...
    selected: list[dict] = []
    seen_pose_keys: set[tuple] = set()
    selected_by_spin: dict[int, int] = {int(round(spin)): 0 for spin in target_spins}
    for target_spin in target_spins:
        target_key = int(round(target_spin))
        matches = [
            item
            for item in ordered
            if _yaw_distance_deg(_variant_yaw_deg_from_labels(item.get("variant_label"), item.get("label")), target_spin) <= 1.0
        ]
        for item in matches:
            if selected_by_spin[target_key] >= per_spin_limit:
                break
            item = dict(item)
            key = (
                _pose_dedupe_key(item.get("pose")),
            ...
            seen_pose_keys.add(key)
            item["_fast_chain_yaw_expansion"] = True
            item["_fast_chain_insert_spin_expansion"] = True
            item["_fast_chain_yaw_expansion_token"] = _fast_chain_insert_spin_token(item, source_name)
            selected.append(item)
            selected_by_spin[target_key] += 1
    if selected:
        spins = [
            int(round(_variant_yaw_deg_from_labels(item.get("variant_label"), item.get("label"))))
            for item in selected
        ...
        for item in selected:
            m = re.search(r"approach_([-+]?\d+(?:\.\d+)?)mm", str(item.get("label", "")).lower())
            approaches.append(int(round(float(m.group(1)))) if m else 0)
        print(
            f"[winner_chain] {source_name or '?'} insert-spin placement batch: "
            f"selected {len(selected)}/{len(ordered)} candidate(s), "
            f"spin_deg={spins}, approach_mm={approaches}"
        )
    return selected


def _fast_chain_insert_spin_token_from_value(spin_deg: float) -> str:
    spin = _fast_chain_normalize_yaw_deg(float(spin_deg))
    if abs(spin) <= 1e-6:
        return "spin_0deg"
    return f"spin_{spin:+.0f}deg"


def _fast_chain_select_single_insert_spin_place_candidate(
    place_candidates,
    target_spin: float,
    source_name: str | None,
) -> dict | None:
    ordered = sorted([dict(item) for item in list(place_candidates or [])], key=_pre_place_screen_sort_key)
    for item in ordered:
        spin = _variant_yaw_deg_from_labels(item.get("variant_label"), item.get("label"))
        if _yaw_distance_deg(spin, target_spin) > 1.0:
            continue
        item["_fast_chain_yaw_expansion"] = True
        item["_fast_chain_insert_spin_expansion"] = True
        item["_fast_chain_yaw_expansion_token"] = _fast_chain_insert_spin_token_from_value(target_spin)
        return item
    return None
...
def _fast_chain_evaluate_paired_relation_records(
    ...
    *,
    ...
    records = _filter_attached_release_consistent_records(records, source_name, label=label)
    if not records:
        return [], {
            "status": "NO_VALID_ATTACHED_RELEASE",
            "paired_relation_count": 0,
            "paired_hover_success_count": 0,
            "paired_release_success_count": 0,
            "paired_both_success_count": 0,
            "paired_pair_count": 0,
            "paired_yaw_expansion_count": 0,
            "paired_yaw_expansion": False,
            "paired_hover_success_relations": [],
            "paired_release_success_relations": [],
            "paired_full_relation_relations": [],
        }
    start_q = np.asarray(start_q, dtype=np.float32).reshape(-1)[:7]
    q_grasps = []
    for rec in records:
    ...
    for rec, hover_result, release_result in zip(records, hover_results, release_results):
        place_relation = rec.get("place_relation_candidate", rec["grasp_candidate"])
    ...
    for grasp_candidate in unique_grasp_candidates:
        ...
        for place_rec in place_records:
            ...
            if bool(rec.get("grasp_specific_pairing", False)) and rec.get("grasp_candidate") is not grasp_candidate:
                continue
...
def _fast_chain_rank_tingzi_pillar_spin_batches(
    planner,
    demo,
    bridge_mod,
    args,
    ...
    *,
    ...
    target_spins = _fast_chain_insert_spin_values(args, source_name)
    if not target_spins:
        return [], {"status": "NO_INSERT_SPINS", "relation_slots": int(relation_slots)}

    relation_candidates = [
        item
        for item in list(grasp_relation_candidates or [])[:relation_slots]
        if item.get("T_tcp_obj") is not None
    ]
    if not relation_candidates:
        return [], {"status": "NO_RELATION_CANDIDATES", "relation_slots": int(relation_slots)}

    place_candidate_cache: dict[int, list[dict]] = {}
    missing_place_count = 0
    all_pair_records: list[dict] = []
    aggregate_debug = {
        "status": "NO_VALID_PAIRED_RELATION",
    ...
    def _merge_tokens(key: str, values) -> None:
        current = list(aggregate_debug.get(key) or [])
        seen = {str(v) for v in current}
        for value in list(values or []):
            text = str(value)
            if text in seen:
                continue
            seen.add(text)
            current.append(text)
        aggregate_debug[key] = current

    def _place_candidates_for(place_relation_candidate: dict, grasp_candidate: dict) -> list[dict]:
        nonlocal missing_place_count
        place_relation_id = id(place_relation_candidate)
        cached = place_candidate_cache.get(place_relation_id)
        ...
        place_candidates = _build_direct_place_candidates(
            ...
            place_state_cache,
            args,
            T_tcp_obj_override=place_relation_candidate.get("T_tcp_obj"),
        )
        if getattr(rule, "primitive", None) == "insert_vertical":
            place_candidates = _filter_pre_place_candidates_by_verticality(place_candidates, args)
        place_candidates = sorted(list(place_candidates or []), key=_pre_place_screen_sort_key)
        if not place_candidates:
            missing_place_count += 1
        place_candidate_cache[place_relation_id] = [dict(item) for item in place_candidates]
        return [dict(item) for item in place_candidates]

    for spin_deg in target_spins:
        spin_records: list[dict] = []
        spin_token = _fast_chain_insert_spin_token_from_value(spin_deg)
        for grasp_candidate in relation_candidates:
            # The object<->TCP transform is fixed once this grasp is selected.
            # Do not borrow the opposite signed tilt relation here: it can make
            # the cheap IK graph pass while the actually attached object is
            # tilted at release.  Axial symmetry is handled by the insertion
            # spin variants generated from the real grasp relation.
            place_candidates = _place_candidates_for(grasp_candidate, grasp_candidate)
            place_candidate = _fast_chain_select_single_insert_spin_place_candidate(
                place_candidates,
                spin_deg,
                source_name,
            )
            if place_candidate is None:
                continue
            spin_records.append(
                {
                    "grasp_candidate": grasp_candidate,
                    "place_relation_candidate": grasp_candidate,
                    ...
                    "relation_token_suffix": spin_token,
                    "yaw_expansion": True,
                    "place_expansion_kind": "insert_spin",
                    "grasp_specific_pairing": True,
                }
            )
        print(
            f"[winner_chain] {source_name or '?'} insert-spin stage batch {spin_token}: "
            f"{len(spin_records)} record(s) from {len(relation_candidates)} grasp relation(s)"
        )
        if not spin_records:
            aggregate_debug["paired_spin_batches"].append(
                {"spin": float(spin_deg), "records": 0, "pairs": 0, "status": "NO_RECORDS"}
            )
            continue
        pair_records, debug = _fast_chain_evaluate_paired_relation_records(
            ...
            start_q,
            source_name=source_name,
            label=f"{label}_{spin_token}",
            disabled_world_collision_links=disabled_world_collision_links,
        )
        all_pair_records.extend(pair_records)
        aggregate_debug["paired_relation_count"] += int(debug.get("paired_relation_count", 0) or 0)
        aggregate_debug["paired_hover_success_count"] += int(debug.get("paired_hover_success_count", 0) or 0)
        aggregate_debug["paired_release_success_count"] += int(debug.get("paired_release_success_count", 0) or 0)
        aggregate_debug["paired_both_success_count"] += int(debug.get("paired_both_success_count", 0) or 0)
        aggregate_debug["paired_pair_count"] += int(debug.get("paired_pair_count", 0) or 0)
        aggregate_debug["paired_yaw_expansion_count"] += int(debug.get("paired_yaw_expansion_count", 0) or 0)
        _merge_tokens("paired_hover_success_relations", debug.get("paired_hover_success_relations"))
        _merge_tokens("paired_release_success_relations", debug.get("paired_release_success_relations"))
        _merge_tokens("paired_full_relation_relations", debug.get("paired_full_relation_relations"))
        aggregate_debug["paired_spin_batches"].append(
            {
                "spin": float(spin_deg),
                "records": int(debug.get("paired_relation_count", 0) or 0),
                "pairs": int(debug.get("paired_pair_count", 0) or 0),
                "status": str(debug.get("status", "")),
            }
        )

    aggregate_debug["missing_place_count"] = int(missing_place_count)
    if all_pair_records:
        aggregate_debug["status"] = "Success"
    return all_pair_records, aggregate_debug
...
def _fast_chain_rank_paired_relation_candidates(
    ...
    *,
    ...
    if relation_slots <= 0:
        return [], {"status": "DISABLED"}
    if _is_tingzi_pillar_source(source_name) and getattr(rule, "primitive", None) == "insert_vertical":
        return _fast_chain_rank_tingzi_pillar_spin_batches(
            planner,
            demo,
            bridge_mod,
            args,
    ...
    missing_relation_count = 0
    start_q = np.asarray(start_q, dtype=np.float32).reshape(-1)[:7]
    relation_candidates = list(grasp_relation_candidates or [])[:relation_slots]
    place_candidate_cache: dict[int, list[dict]] = {}
    for grasp_candidate in relation_candidates:
        if grasp_candidate.get("T_tcp_obj") is None:
            missing_relation_count += 1
            continue
        place_relation_candidates = [grasp_candidate]
        if _is_tingzi_pillar_source(source_name) and getattr(rule, "primitive", None) == "insert_vertical":
            grasp_match_key = _fast_chain_relation_match_key(grasp_candidate, source_name)
            place_relation_candidates = [
                item
                for item in relation_candidates
                if item.get("T_tcp_obj") is not None
                and _fast_chain_relation_match_key(item, source_name) == grasp_match_key
            ]
            place_relation_candidates.sort(
                key=lambda item: (
                    0 if item is grasp_candidate else 1,
                    str(item.get("label", "")),
                )
            )

        for place_relation_candidate in place_relation_candidates:
            place_relation_id = id(place_relation_candidate)
            place_candidates = place_candidate_cache.get(place_relation_id)
            ...
            if not place_candidates:
                missing_place_count += 1
                continue
            batched_place_candidates = []
            if _is_tingzi_pillar_source(source_name) and getattr(rule, "primitive", None) == "insert_vertical":
                batched_place_candidates = _fast_chain_select_insert_spin_place_candidates(
                    place_candidates,
                    args,
                    source_name,
                )
            if not batched_place_candidates:
                batched_place_candidates = [dict(place_candidates[0])]
            place_relation_token = _fast_chain_relation_summary_token(place_relation_candidate)
            grasp_relation_token = _fast_chain_relation_summary_token(grasp_candidate)
            relation_sign_suffix = ""
            if (
                _is_tingzi_pillar_source(source_name)
                and place_relation_candidate is not grasp_candidate
                and place_relation_token != grasp_relation_token
            ):
                relation_sign_suffix = f"place_{place_relation_token}"
            for place_candidate in batched_place_candidates:
                suffix_parts = []
                if relation_sign_suffix:
                    suffix_parts.append(relation_sign_suffix)
                if bool(place_candidate.get("_fast_chain_insert_spin_expansion", False)):
                    suffix_parts.append(str(place_candidate.get("_fast_chain_yaw_expansion_token", "") or ""))
                suffix = "@".join([part for part in suffix_parts if part])
                records.append(
                    {
                        "grasp_candidate": grasp_candidate,
                        "place_relation_candidate": place_relation_candidate,
                        "place_candidate": dict(place_candidate),
                        "place_candidates": [dict(item) for item in place_candidates],
                        "place_candidate_count": len(place_candidates),
                        "relation_token_suffix": suffix,
                        "yaw_expansion": bool(place_candidate.get("_fast_chain_insert_spin_expansion", False)),
                        "place_expansion_kind": "insert_spin" if suffix else "",
                    }
                )
    ...
    *,
    ...
    with _profile_stage(
        ...
        if not pair_records:
            ...
            return None
        if _is_tingzi_pillar_source(source_name):
            pair_records.sort(key=_tingzi_pillar_fast_chain_pair_sort_key)
        else:
...
def _evaluate_joint_grasp_place_chains(
    ...
    # Once the source object is attached, keeping its original world obstacle creates
    # a duplicate collision model and can invalidate the transport start state.
    #
    # Do not exclude the insertion target (for the pavilion this is tingzi_base).
    # It must remain in the cuRobo world for transport/start-lift avoidance; the
    # final insertion step handles target contact separately with a reduced local
    # planner box.
...
def run_targeted_place_episode_curobo_direct(
    ...
    if real_exec is not None:
        ...
        with _profile_stage(
            ...
            try:
                ...
                prof["blocked_before_full_close"] = blocked
                args._last_real_gripper_pos_after_close = gripper_pos
                args._last_real_gripper_blocked_after_close = blocked
    ...
    wrist_relation_job = _start_wrist_relation_after_grasp(demo, args)
    ...
    wrist_relation_applied = False
    wrist_relation_deferred_to_transport = _wrist_relation_during_transport_enabled(args)
    if wrist_relation_deferred_to_transport and wrist_relation_job is not None:
        print("[wrist_relation] deferring relation apply until original pre_place hover; estimator runs during transport")
        _record_profile(args, "wrist_relation_apply", success=False, status="DEFERRED_TO_PRE_PLACE", phase="before_place_planning")
    else:
        wrist_relation_applied = _wait_and_apply_wrist_relation(
            demo,
            args,
            planner,
            wrist_relation_job,
            label="before_place_planning",
        )
        if wrist_relation_applied and selected_joint_chain is not None:
            print(
                "[wrist_relation] refined TCP-object relation invalidates the precomputed "
                "joint_search transport/pre_place chain; replanning place from current state"
            )
            selected_joint_chain = None
    ...
    # During transport/final place, only the grasped source is represented by
    # attached payload spheres.  Already placed assembly parts and the insertion
    # target stay in the world so paths cannot sweep through them.
    ...
    if (
        wrist_relation_deferred_to_transport
        and wrist_relation_job is not None
        and bool(place_choice.get("two_stage_place", False))
        and len(q_pre_place_path) >= 1
    ):
        print(
            "[wrist_relation] executing original pre_place hover while wrist relation finishes; "
            "precise pre_place_new will be planned at hover if accepted"
        )
        _mark_real_motion_started("place_hover_original_for_wrist_refine")
        with _profile_stage(
            args,
            "place_hover_original_for_wrist_refine",
            label=f"{place_choice['label']}_original_hover",
            ...
            ok, _ = targeted.base.execute_pose_path_stage(
                ...
                bridge_mod,
                real_exec,
                f"{place_choice['label']}_original_hover",
        ...
        original_hover_q = np.asarray(demo.current_arm_qpos(), dtype=np.float32).reshape(-1)[:7]
        wait_at_hover_s = float(max(getattr(args, "wrist_relation_wait_at_pre_place_s", 0.75), 0.0))
        wrist_relation_applied = _wait_and_apply_wrist_relation(
            demo,
            args,
            planner,
            wrist_relation_job,
            label="at_original_pre_place",
            timeout_s=wait_at_hover_s,
        )
        refined_choice = None
        refined_q_pre_place_path: list[np.ndarray] = []
        refined_q_place_path: list[np.ndarray] = []
        if wrist_relation_applied:
            refined_choice, refined_q_pre_place_path, refined_q_place_path = _plan_wrist_refined_place_from_current_hover(
                planner=planner,
                demo=demo,
                bridge_mod=bridge_mod,
                args=args,
                ...
                rule=rule,
                place_state_cache=place_state_cache,
                direct_place_disabled_links=direct_place_disabled_links,
                exclude_names_place=exclude_names_place,
            )
        if refined_choice is not None:
            place_choice = refined_choice
            q_pre_place_path = refined_q_pre_place_path
            q_place_path = refined_q_place_path
            print(
                "[wrist_relation] using refined pre_place_new before insertion "
                f"(pre_waypoints={len(q_pre_place_path)}, place_waypoints={len(q_place_path)})"
            )
        else:
            if wrist_relation_applied and bool(getattr(args, "wrist_relation_require_refined_place", False)):
                print("[wrist_relation][FAIL] refined relation was applied but pre_place_new planning failed")
                if planner.attached_object_active:
                    planner.detach_object_from_robot()
                return False
            print("[wrist_relation] continuing from original hover with the existing final-contact path")
            if q_place_path:
                q_place_path = [np.asarray(q, dtype=np.float32).reshape(-1)[:7] for q in q_place_path]
                if np.allclose(original_hover_q, q_place_path[0], atol=0.05, rtol=0.0):
                    q_place_path[0] = original_hover_q.copy()
            q_pre_place_path = [original_hover_q.copy()]
            place_choice["q_pre_place_path"] = q_pre_place_path
            place_choice["q_place_path"] = q_place_path
            place_choice["q_path"] = q_pre_place_path + q_place_path[1:] if q_place_path else q_pre_place_path
    ...
    insert_partial_release = (
        place_mode_name == "insert_place"
        and bool(getattr(args, "insert_place_partial_open_at_release", False))
    )
    release_gripper_pos = float(args.real_gripper_open)
    release_gripper_reading = None
    if insert_partial_release:
        try:
            if real_exec is not None:
                release_gripper_reading = real_exec.get_gripper_pos()
        except Exception:
            release_gripper_reading = None
        if release_gripper_reading is None:
            release_gripper_reading = getattr(args, "_last_real_gripper_pos_after_close", None)
        release_gripper_pos = _insert_place_partial_open_command(args, release_gripper_reading)
        print(
            "[open gripper at place] insert partial release: "
            f"current={release_gripper_reading}, partial_cmd={release_gripper_pos:.3f}, "
            f"full_open_cmd={float(args.real_gripper_open):.3f}"
        )

    deferred_insert_full_open = bool(
        insert_partial_release
        and bool(getattr(args, "insert_place_full_open_after_clearance", True))
        and abs(float(release_gripper_pos) - float(args.real_gripper_open)) > 1e-6
    )

    def _finish_deferred_insert_full_open(reason: str) -> bool:
        nonlocal deferred_insert_full_open
        if not deferred_insert_full_open:
            return True
        print(f"[open gripper after insert clearance] full open after {reason}")
        if real_exec is not None:
            _mark_real_motion_started("gripper_full_open_after_insert_clearance")
            with _profile_stage(
                args,
                "insert_place_full_open_after_clearance",
                reason=str(reason),
                gripper_pos=float(args.real_gripper_open),
                previous_gripper_pos=float(release_gripper_pos),
                real_exec=True,
                repeats=int(getattr(args, "real_gripper_command_repeats", 2)),
                hz=float(getattr(args, "real_gripper_command_hz", 10.0)),
                sim_min_steps=int(getattr(args, "sim_gripper_sync_min_steps", 8)),
            ...
            with _profile_stage(
                ...
                prof["success"] = True
                prof["status"] = "DRY_RUN"
        deferred_insert_full_open = False
        return True
    ...
    if real_exec is not None:
        ...
        with _profile_stage(
            args,
            "place_open_gripper",
            gripper_pos=float(release_gripper_pos),
            full_open_gripper_pos=float(args.real_gripper_open),
            partial_insert_release=bool(insert_partial_release),
            current_gripper_pos=release_gripper_reading,
            full_open_deferred=bool(deferred_insert_full_open),
            real_exec=True,
            repeats=int(getattr(args, "real_gripper_command_repeats", 2)),
            hz=float(getattr(args, "real_gripper_command_hz", 10.0)),
            sim_min_steps=int(getattr(args, "sim_gripper_sync_min_steps", 8)),
            ...
            try:
                real_exec.set_gripper(release_gripper_pos)
                if insert_partial_release:
                    _sync_demo_gripper_to_real_command(
                        demo,
                        args,
                        release_gripper_pos,
                        steps=int(getattr(args, "insert_place_partial_open_sim_steps", 2)),
                        label="partial opened",
                    )
                else:
                    targeted.base.sync_demo_gripper_state(demo, closed=False, steps=4)
                _mark_object_released()
    ...
    if q_clearance_path:
        ...
        clearance_gripper_pos = float(release_gripper_pos) if deferred_insert_full_open else float(args.real_gripper_open)
        ok, _ = targeted.base.execute_pose_path_stage(
            demo,
            bridge_mod,
            real_exec,
    ...
    if clearance_executed:
        _finish_deferred_insert_full_open("post_place_clearance")

    if not settled_before_clearance:
        targeted.base.settle_released_active_object_for_scene_cache(demo, args)
    _record_placed_pose_prediction_error(args, demo, place_choice)
    _stabilize_assembly_place_cache(args, demo, scene_capture_cache, place_choice)
    ...
    if bool(getattr(args, "skip_return_to_cycle_start", False)):
        if force_clearance_after_insert and not clearance_executed:
            ...
            return_ok = _plan_and_execute_return_to_cycle_start(
                ...
                args,
                start_q,
                gripper_pos=float(release_gripper_pos) if deferred_insert_full_open else None,
            )
            if return_ok:
                _finish_deferred_insert_full_open("return_to_cycle_start")
...
def _single_scene_build_obstacle_entry(demo, args, object_name: str, meta: dict, scene_capture_cache) -> dict | None:
    ...
    cache_T_world_obj = None
    if isinstance(cache_entry, dict) and cache_entry.get("T_world_obj") is not None:
        try:
            cache_T_world_obj = np.asarray(cache_entry["T_world_obj"], dtype=np.float32).reshape(4, 4)
        except Exception:
            cache_T_world_obj = None
    actor_T_world_obj = _single_scene_actor_pose_matrix(actor)
    if placed and cache_T_world_obj is not None:
        T_world_obj = cache_T_world_obj
        if actor is not None:
            try:
                actor.set_pose(_pose_from_world_matrix(T_world_obj))
                _single_scene_zero_actor_velocity(actor)
        ...
        T_world_obj = actor_T_world_obj
        if T_world_obj is None and cache_T_world_obj is not None:
            T_world_obj = cache_T_world_obj
    if T_world_obj is None:
        return None
...
def _single_scene_sync_obstacles(demo, args, selected_name: str, obstacle_names, scene_capture_cache) -> None:
    ...
    desired_names = {
        ...
        and not targeted.is_virtual_place_target(name)
    }
    if selected_name is not None:
        desired_names.discard(selected_name)
    if selected_name == "tingzi_base":
        desired_names = {
            name
            for name in desired_names
            if not str(name or "").startswith("tingzi_pillar_")
        }
...
def _should_use_predicted_place_pose_for_assembly_cache(object_name: str | None) -> bool:
    normalized = curobo_wrapper.normalize_object_name(object_name)
    return bool(normalized and normalized.startswith("tingzi_"))


def _snap_cached_placed_assembly_actors(demo, scene_capture_cache, *, refresh: bool = True) -> int:
    if not isinstance(scene_capture_cache, dict):
        return 0
    objects = scene_capture_cache.get("objects")
    if not isinstance(objects, dict):
        return 0
    registry = _single_scene_registry(demo)
    active_name = curobo_wrapper.normalize_object_name(getattr(getattr(demo, "args", None), "object_name", None))
    snapped = 0
    for raw_name, entry in objects.items():
        name = curobo_wrapper.normalize_object_name(raw_name)
        if not name or not name.startswith("tingzi_"):
            continue
        if not isinstance(entry, dict) or not bool(entry.get("placed", False)) or entry.get("T_world_obj") is None:
            continue
        try:
            T_world_obj = np.asarray(entry["T_world_obj"], dtype=np.float32).reshape(4, 4)
        except Exception:
            continue
        actor = (registry.get(name) or {}).get("actor")
        if actor is None and name == active_name:
            actor = getattr(getattr(demo, "base_env", None), "obj", None)
        if actor is not None:
            try:
                actor.set_pose(_pose_from_world_matrix(T_world_obj))
                ...
                pass
        for obstacle in list(getattr(demo, "scene_obstacles", []) or []):
            if curobo_wrapper.normalize_object_name(obstacle.get("object_name")) != name:
                continue
            obstacle["T_world_obj"] = T_world_obj.copy()
            _single_scene_set_planner_obstacle(demo, obstacle)
    if snapped and bool(refresh):
        _single_scene_refresh_after_pose_change(demo)
    return snapped


def _lock_cached_placed_assembly_actors_for_render(demo) -> None:
    scene_capture_cache = getattr(demo, "scene_capture_cache_ref", None)
    if not isinstance(scene_capture_cache, dict):
        return
    try:
        _snap_cached_placed_assembly_actors(demo, scene_capture_cache, refresh=False)
    except Exception:
        pass


def _stabilize_assembly_place_cache(args, demo, scene_capture_cache, place_choice) -> None:
    object_name = curobo_wrapper.normalize_object_name(getattr(args, "object_name", None))
    if not _should_use_predicted_place_pose_for_assembly_cache(object_name):
        return
    predicted_T = _predicted_place_T_world_obj(place_choice)
    if predicted_T is None:
        return
    predicted_T = np.asarray(predicted_T, dtype=np.float32).reshape(4, 4).copy()
    ...
    if actual_T is not None:
        try:
            ...
            delta_m = float(np.linalg.norm(actual_T[:3, 3] - predicted_T[:3, 3]))
            ...
            delta_m = None
            delta_deg = None
    cached_T = _apply_predicted_place_to_scene_cache(scene_capture_cache, object_name, args, place_choice)
    if cached_T is None:
        return
    try:
        demo._scene_cache_placed_object_world_pose = predicted_T.copy()
    except Exception:
        pass
    actor = getattr(getattr(demo, "base_env", None), "obj", None)
    if actor is not None:
        try:
            actor.set_pose(_pose_from_world_matrix(predicted_T))
            _single_scene_zero_actor_velocity(actor)
        except Exception as exc:
            print(f"[scene cache] warning: failed to snap placed {object_name} actor to planned pose: {exc}")
    snapped_count = _snap_cached_placed_assembly_actors(demo, scene_capture_cache)
    _record_profile(
        args,
        "assembly_place_cache_stabilize",
        success=True,
        status="PREDICTED_POSE_AS_TRUTH",
        target_name=object_name,
        predicted_xyz=[float(v) for v in predicted_T[:3, 3].tolist()],
        actual_predicted_delta_m=delta_m,
        actual_predicted_delta_deg=delta_deg,
    )
    delta_text = "" if delta_m is None else f" (actual-plan delta {delta_m:.4f} m"
    if delta_text and delta_deg is not None:
        delta_text += f", {delta_deg:.1f} deg"
    if delta_text:
        delta_text += ")"
    print(
        f"[scene cache] stabilized placed {object_name} at planned assembly pose "
        f"{np.round(predicted_T[:3, 3], 6).tolist()}{delta_text}; "
        f"snapped_placed_actors={snapped_count}"
    )
        ...
        try:
            ...
            with _profile_record_context(
                ...
                prefetch_args.planning_profile_dir = getattr(
                    ...
                    str(RUNTIME_DIR / "planning_profile_logs"),
                )
...
def _render_dry_run_motion_frame(demo, bridge_mod, args) -> None:
    if getattr(args, "render_mode", None) != "human":
        return
    try:
        _lock_cached_placed_assembly_actors_for_render(demo)
        bridge_mod.render_preview(demo.env, repeats=1)
        _lock_cached_placed_assembly_actors_for_render(demo)
    except Exception:
        pass
...
def _install_cuda_render_sync_wrappers() -> None:
    ...
    if callable(original_sync_demo_arm_qpos):
        ...
        def _sync_demo_arm_qpos_serialized(*args, **kwargs):
            with _CUROBO_GPU_LOCK:
                result = original_sync_demo_arm_qpos(*args, **kwargs)
                if args:
                    _lock_cached_placed_assembly_actors_for_render(args[0])
                return result
