...
from rm75_app.assets.object_specs import describe_object_specs, get_object_spec, list_object_spec_names, normalize_object_name, resolve_object_spec_scales
from rm75_app.paths import APP_ROOT, DEFAULT_CAMERA_EXTRINSIC, DEFAULT_CUROBO_CFG, RUNTIME_DIR


PACKAGE_DIR = APP_ROOT / "rm75_app"
DEFAULT_BRIDGE_SCRIPT = str(PACKAGE_DIR / "execution" / "rm75_bridge.py")
DEFAULT_PICK_SCRIPT = str(PACKAGE_DIR / "execution" / "pick_move_v10.py")
DEFAULT_LEROBOT_ROOT = str(APP_ROOT)
DEFAULT_LEROBOT_SIM2REAL_ROOT = str(APP_ROOT.parent / "lerobot-sim2real")
...
DEFAULT_CAMERA_EXTRINSIC_PATH = str(DEFAULT_CAMERA_EXTRINSIC)
...
def patch_maniskill_compute_angle_between_device_mismatch() -> None:
    ...
    if callable(original) and not getattr(original, "_rm75_device_patch", False):
        def compute_angle_between_same_device(x1, x2):
            ...
            if isinstance(x1, torch.Tensor):
                target_device = x1.device
            if isinstance(x2, torch.Tensor) and (target_device is None or getattr(x2, "is_cuda", False)):
                target_device = x2.device
            if not isinstance(x1, torch.Tensor):
                x1 = torch.as_tensor(x1, dtype=torch.float32, device=target_device)
            elif target_device is not None:
                x1 = x1.to(device=target_device)
            if not isinstance(x2, torch.Tensor):
                x2 = torch.as_tensor(x2, dtype=x1.dtype if isinstance(x1, torch.Tensor) else torch.float32, device=target_device)
            elif target_device is not None:
                x2 = x2.to(device=target_device)
            if x1.dtype != x2.dtype and x1.dtype in (torch.float16, torch.float32, torch.float64):
    ...
    def is_grasping_same_device(self, object, min_force=0.5, max_angle=95):
        def _as_torch_tensor(value, *, device=None):
        ...
        target_device = getattr(l_contact_forces, "device", None)
        l_contact_forces = _as_torch_tensor(l_contact_forces, device=target_device)
        target_device = l_contact_forces.device
        r_contact_forces = _as_torch_tensor(r_contact_forces, device=target_device)
        lforce = torch.linalg.norm(l_contact_forces, dim=1)
        rforce = torch.linalg.norm(r_contact_forces, dim=1)

        ldirection = _as_torch_tensor(
            self.finger1_link.pose.to_transformation_matrix()[..., :3, 1],
            device=target_device,
        )
        rdirection = -_as_torch_tensor(
            self.finger2_link.pose.to_transformation_matrix()[..., :3, 1],
            device=target_device,
        )
    ...
    globals()["_RM75_SAFE_IS_GRASPING"] = is_grasping_same_device
    try:
        import importlib
...
def patch_runtime_agent_is_grasping(env) -> None:
    safe_is_grasping = globals().get("_RM75_SAFE_IS_GRASPING")
    if safe_is_grasping is None:
    ...
    try:
        ...
        if agent is None:
            return
        current = getattr(agent, "is_grasping", None)
        if callable(current) and getattr(current, "_rm75_device_patch", False):
            return
        agent.is_grasping = types.MethodType(safe_is_grasping, agent)
    except Exception:
        pass


def patch_runtime_reward_tensor_fields(env) -> None:
    try:
        import torch
        base_env = getattr(env, "unwrapped", env)
        value = getattr(base_env, "obj_xy_shortest_edge_vector", None)
        if value is not None and not isinstance(value, torch.Tensor):
            device = getattr(base_env, "device", None)
            setattr(
                base_env,
                "obj_xy_shortest_edge_vector",
...
def build_arg_parser():
    ...
    parser.add_argument("--camera-extrinsic-opencv-path", type=str, default=DEFAULT_CAMERA_EXTRINSIC_PATH)
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug-dir",
        type=str,
        default=str(RUNTIME_DIR / "debug_jiaobang_foundationpose_real"),
    )
    ...
    parser.add_argument(
        "--failure-render-dir",
        type=str,
        default=str(RUNTIME_DIR / "failure_renders"),
...
def build_grasp_pose_variants(demo, grasp_pose, args):
    ...
    grasp_mode = str(getattr(args, "grasp_mode", "object_normal") or "object_normal").strip().lower()
    if grasp_mode not in {"topdown_long_axis", "long_axis_adaptive"}:
        return variants
...
def build_foundationpose_scene_cache_key(args):
    ...
    return (
        ...
        str(Path(getattr(args, "camera_extrinsic_opencv_path", DEFAULT_CAMERA_EXTRINSIC_PATH)).expanduser()),
...
def _load_fixed_scene_capture(args, bridge_mod):
    ...
    virtual_targets_raw = data.get("virtual_targets", {})
    if virtual_targets_raw is None:
        virtual_targets_raw = {}
    if not isinstance(virtual_targets_raw, dict):
        raise ValueError(f"Invalid fixed scene file {scene_file}: top-level 'virtual_targets' must be a dictionary")
    ...
    normalized_virtual_targets = {
        normalize_object_name(name): entry
        for name, entry in virtual_targets_raw.items()
        if normalize_object_name(name) is not None and isinstance(entry, dict)
    }
    ...
    for object_name in selected_obstacle_names:
        if object_name in normalized_virtual_targets:
            continue
    ...
    return {
        ...
        "virtual_targets": normalized_virtual_targets,
    }
            ...
            scene_capture_cache.update(
                {
                    ...
                    "virtual_targets": fixed_scene.get("virtual_targets", {}),
                }
            )
        return (
            fixed_scene["fp_rt"],
...
def create_demo(args, bridge_mod, planner_mod, scene_capture_cache=None):
    ...
    patch_runtime_agent_is_grasping(env)
    patch_runtime_reward_tensor_fields(env)
...
def _failure_render_output_path(args, label: str, *, suffix: str = "") -> Path:
    output_dir = Path(getattr(args, "failure_render_dir", RUNTIME_DIR / "failure_renders")).expanduser()
