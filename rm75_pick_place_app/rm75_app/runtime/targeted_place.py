...
from rm75_app.runtime import foundationpose_scene as base
from rm75_app.assets.object_specs import normalize_object_name
from rm75_app.placement.place_rules import (
    LocalPoseSpec,
    PlaceRule,
    ...
    is_virtual_place_target,
    install_dynamic_place_rules_from_file,
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable roof-assembly diagnostics/defaults for red triangle panels on the brick cube.",
    )
    parser.add_argument("--roof-side", choices=["auto", "front", "back", "left", "right"], default="auto")
    parser.add_argument("--roof-cube-name", default="red_bricks_cube")
    ...
    parser.add_argument(
        "--roof-tip-grasp-world-offset",
        type=float,
        nargs=3,
        default=[0.0, 0.0, 0.0],
        help=(
            "Accepted by the roof scene profile for compatibility with grasp tuning. "
            "The current place primitive uses the measured TCP-object relation after grasp."
        ),
    )
    parser.add_argument(
        "--roof-preplace-outward-offset-m",
        type=float,
        default=0.055,
        help="Roof primitive pre-place offset along the outward face normal before final contact.",
    )
    parser.add_argument(
        "--roof-hover-extra-z-m",
        type=float,
        default=0.030,
        help="Roof primitive extra world-Z lift at pre-place.",
    )
    parser.add_argument(
        "--roof-release-outward-offset-m",
        type=float,
        default=0.006,
        help="Small outward release offset for the roof panel target pose.",
    )
    parser.add_argument(
...
def _ensure_target_registered_for_place(demo, target_name: str | None) -> None:
    ...
    if is_virtual_place_target(normalized):
        return
...
def _get_scene_object_world_transform(demo, bridge_mod, scene_capture_cache, object_name: str | None) -> np.ndarray | None:
    ...
    if not isinstance(scene_capture_cache, dict):
        return None
    virtual_targets = scene_capture_cache.get("virtual_targets")
    if isinstance(virtual_targets, dict):
        virtual_item = virtual_targets.get(normalized)
        if isinstance(virtual_item, dict) and virtual_item.get("T_world_obj") is not None:
            return np.asarray(virtual_item["T_world_obj"], dtype=np.float32).reshape(4, 4)
    ...
    if item.get("T_world_obj") is not None:
        return np.asarray(item["T_world_obj"], dtype=np.float32).reshape(4, 4)
    ...
    *,
    ...
    for family_label, T_family in base_families:
        ...
        for spin_deg in spin_degs:
            ...
            spin_axis = getattr(rule, "insert_spin_axis_local", None)
            if spin_axis is None:
                spin_axis = (0.0, 1.0, 0.0)
            spin_axis = _normalize(np.asarray(spin_axis, dtype=np.float32).reshape(3))
            if spin_axis is None:
                spin_axis = np.asarray([0.0, 1.0, 0.0], dtype=np.float32)
            T_spin[:3, :3] = _axis_angle_to_matrix(spin_axis, np.deg2rad(float(spin_deg))).astype(np.float32)
...
def _target_place_up_axis(rule: PlaceRule, T_world_target: np.ndarray) -> np.ndarray | None:
    if is_virtual_place_target(getattr(rule, "target_object_name", None)):
        return np.array([0.0, 0.0, 1.0], dtype=np.float32)
    if rule.primitive == "place_on_slots" and normalize_object_name(rule.target_object_name) == "desk":
        return np.array([0.0, 0.0, 1.0], dtype=np.float32)
    if rule.primitive == "roof_assembly":
        return _normalize(np.asarray(T_world_target[:3, 2], dtype=np.float32).reshape(3))
    if rule.primitive == "insert_vertical" and getattr(rule, "insert_axis_local", None) is not None:
        axis_local = np.asarray(getattr(rule, "insert_axis_local"), dtype=np.float32).reshape(3)
        return _normalize(np.asarray(T_world_target[:3, :3], dtype=np.float32).reshape(3, 3) @ axis_local)
    return _normalize(np.asarray(T_world_target[:3, 1], dtype=np.float32).reshape(3))


def _infer_roof_side(rule: PlaceRule, args) -> str:
    requested = str(getattr(args, "roof_side", "auto") or "auto").strip().lower()
    if requested in {"front", "back", "left", "right"}:
        return requested
    name = str(getattr(rule, "source_object_name", "") or "").lower()
    for side in ("front", "back", "left", "right"):
        if side in name:
            return side
    raise RuntimeError(
        f"Cannot infer roof side from source object {rule.source_object_name!r}; "
        "pass --roof-side front/back/left/right."
    )


def _orthonormal_frame_from_axes(primary: np.ndarray, secondary: np.ndarray) -> np.ndarray:
    x_axis = _normalize(primary)
    if x_axis is None:
        raise ValueError("roof frame primary axis is zero")
    y_axis = np.asarray(secondary, dtype=np.float32).reshape(3)
    y_axis = y_axis - x_axis * float(np.dot(y_axis, x_axis))
    y_axis = _normalize(y_axis)
    if y_axis is None:
        raise ValueError("roof frame secondary axis is parallel to primary")
    z_axis = _normalize(np.cross(x_axis, y_axis))
    if z_axis is None:
        raise ValueError("roof frame normal axis is zero")
    y_axis = _normalize(np.cross(z_axis, x_axis))
    if y_axis is None:
        raise ValueError("roof frame re-orthogonalized secondary axis is zero")
    return np.stack([x_axis, y_axis, z_axis], axis=1).astype(np.float32)


def _roof_side_axes(T_world_cube: np.ndarray, side: str) -> tuple[np.ndarray, np.ndarray]:
    x_axis = _normalize(T_world_cube[:3, 0])
    y_axis = _normalize(T_world_cube[:3, 1])
    if x_axis is None or y_axis is None:
        raise ValueError("roof cube pose has invalid axes")
    if side == "front":
        return x_axis, (-y_axis).astype(np.float32)
    if side == "back":
        return (-x_axis).astype(np.float32), y_axis
    if side == "left":
        return (-y_axis).astype(np.float32), (-x_axis).astype(np.float32)
    if side == "right":
        return y_axis, x_axis
    raise ValueError(f"invalid roof side: {side}")


def _roof_assembly_object_pose(
    rule: PlaceRule,
    T_world_target: np.ndarray,
    args,
) -> tuple[str, np.ndarray, np.ndarray, np.ndarray]:
    target_name = normalize_object_name(getattr(rule, "target_object_name", None))
    configured_cube_name = normalize_object_name(getattr(args, "roof_cube_name", "red_bricks_cube"))
    if configured_cube_name is not None and target_name is not None and configured_cube_name != target_name:
        print(
            f"[roof_assembly] warning: rule target={target_name} differs from "
            f"--roof-cube-name={configured_cube_name}; using rule target pose"
        )
    T_world_target = np.asarray(T_world_target, dtype=np.float32).reshape(4, 4)
    side = _infer_roof_side(rule, args)
    edge_axis_world, outward_axis_world = _roof_side_axes(T_world_target, side)
    target_up_axis = _target_place_up_axis(rule, T_world_target)
    if target_up_axis is None:
        raise ValueError("roof target up axis is invalid")

    cube_size = float(max(getattr(args, "roof_cube_size_m", 0.105), 1e-5))
    roof_height = float(max(getattr(args, "roof_height_m", 0.080), 1e-5))
    release_outward = float(getattr(args, "roof_release_outward_offset_m", 0.006))
    cube_center = np.asarray(T_world_target[:3, 3], dtype=np.float32).reshape(3)
    base_center_world = (
        cube_center
        + outward_axis_world * (cube_size * 0.5 + release_outward)
        + target_up_axis * (cube_size * 0.5)
    ).astype(np.float32)
    peak_world = (cube_center + target_up_axis * (cube_size * 0.5 + roof_height)).astype(np.float32)
    apex_axis_world = _normalize(peak_world - base_center_world)
    if apex_axis_world is None:
        raise ValueError("roof apex axis is invalid")

    local_edge_axis = np.asarray(getattr(args, "roof_triangle_local_edge_axis", [1.0, 0.0, 0.0]), dtype=np.float32).reshape(3)
    local_up_axis = np.asarray(getattr(args, "roof_triangle_local_up_axis", [0.0, 1.0, 0.0]), dtype=np.float32).reshape(3)
    local_base_center = np.asarray(getattr(args, "roof_triangle_base_center_local", [0.0, 0.0, 0.0]), dtype=np.float32).reshape(3)
    B_local = _orthonormal_frame_from_axes(local_edge_axis, local_up_axis)
    B_world = _orthonormal_frame_from_axes(edge_axis_world, apex_axis_world)
    R_world_obj = (B_world @ np.linalg.inv(B_local)).astype(np.float32)

    T_world_obj = np.eye(4, dtype=np.float32)
    T_world_obj[:3, :3] = R_world_obj
    T_world_obj[:3, 3] = (base_center_world - R_world_obj @ local_base_center).astype(np.float32)
    return side, T_world_obj, outward_axis_world.astype(np.float32), target_up_axis.astype(np.float32)


def _build_roof_assembly_plan_variants(
    demo,
    rule: PlaceRule,
    T_world_target: np.ndarray,
    target_name: str,
    ...
    side, T_world_obj_desired, outward_axis_world, target_up_axis = _roof_assembly_object_pose(rule, T_world_target, args)
    T_world_tcp_place = T_world_obj_desired @ np.linalg.inv(np.asarray(T_tcp_obj, dtype=np.float32).reshape(4, 4))
    place_pose = _pose_from_matrix(T_world_tcp_place)
    p_place = base.flatten_np(place_pose.p)[:3].astype(np.float32)
    preplace_outward = float(max(getattr(args, "roof_preplace_outward_offset_m", 0.055), 0.0))
    hover_z = float(max(getattr(args, "roof_hover_extra_z_m", 0.030), 0.0))
    pre_place_pose = base.make_pose_with_position(
        place_pose,
        (
            p_place
            + outward_axis_world * preplace_outward
            + target_up_axis * hover_z
        ).astype(np.float32),
    )
    retreat_pose = pre_place_pose
    tcp_approach_axis = _normalize(np.asarray(T_world_tcp_place[:3, 2], dtype=np.float32).reshape(3))
    tcp_verticality = 0.0 if tcp_approach_axis is None else abs(float(np.dot(tcp_approach_axis, target_up_axis)))
    variant_label = f"roof_{side}"
    print(
        f"[roof_assembly] source={rule.source_object_name}, side={side}, target={target_name}, "
        f"base={np.round(T_world_obj_desired[:3, 3], 5).tolist()}, "
        f"preplace_out={preplace_outward:.3f}, hover_z={hover_z:.3f}"
    )
    return [
        TargetedPlacePlan(
            rule=rule,
            ...
            T_world_obj_desired=T_world_obj_desired.astype(np.float32),
            staging_pose=None if not bool(getattr(args, "targeted_place_staging", True)) else _build_targeted_place_staging_pose(demo, pre_place_pose, args),
...
def _make_tabletop_place_world_pose_variants(
    ...
    use_world_yaw_variants = bool(getattr(rule, "world_tabletop_yaw_variants", False))
    ...
    elif bool(getattr(rule, "allow_tabletop_yaw_variants", False)) or use_world_yaw_variants:
    ...
    if (
        bool(getattr(rule, "allow_tabletop_yaw_variants", False))
        and not use_world_yaw_variants
        and not use_carriot_relative_yaw
    ):
        ref_axis = None
    ...
    for yaw_deg in yaw_degs:
        ...
        elif use_world_yaw_variants:
            yaw_label = None if abs(yaw_deg) <= 1e-6 else f"world_yaw_{int(round(yaw_deg))}deg"
        else:
            yaw_label = None if abs(yaw_deg) <= 1e-6 else f"world_yaw_{int(round(yaw_deg))}deg"
        ...
        for tilt_deg in tilt_degs:
            ...
            seen.add(key)
            variants.append((label, T_variant))
    fallback_label = None if (use_world_yaw_variants or not bool(getattr(rule, "allow_tabletop_yaw_variants", False))) else "face_robot"
    return variants or [(fallback_label, T_world_obj_base)]
...
def build_targeted_place_plan_variants(
    ...
    *,
    ...
    if rule.primitive == "roof_assembly":
        return _build_roof_assembly_plan_variants(
            demo,
            rule,
            T_world_target,
            target_name,
...
def _set_scene_obstacle_planner_box_scale(demo, object_name: str, scale: float) -> bool:
    ...
    if "_original_planner_box_size" not in scene_entry:
        scene_entry["_original_planner_box_size"] = np.asarray(planner_box_size, dtype=np.float32).reshape(3).copy()
    planner_box_size = np.asarray(scene_entry.get("_original_planner_box_size", planner_box_size), dtype=np.float32).reshape(3)
    scale = float(max(scale, 1e-3))
    scaled_box_size = np.maximum(planner_box_size * scale, 1e-4).astype(np.float32)
    scene_entry["planner_box_size"] = scaled_box_size.copy()
...
def _cached_placed_object_names(scene_capture_cache) -> list[str]:
    if not isinstance(scene_capture_cache, dict):
        return []
    objects = scene_capture_cache.get("objects")
    if not isinstance(objects, dict):
...
def _derive_cycle_obstacle_names(
    base_args,
    cycle_idx: int,
    selected_name: str,
    ...
    if selected_name is not None and selected_name.startswith("tingzi_"):
        current_rule_target_names = get_required_place_target_names([selected_name])
        if selected_name == "tingzi_base" and cycle_idx == 1:
            # Cycle 1 still requests the loose pillars so their actors are
            # created in the shared ManiSkill scene; direct_pre_place removes
            # them from the planner collision world before grasp planning.
            return _dedupe_names(explicit_names + tracked_scene_names + future_cycle_sources + current_rule_target_names)
        placed_names = [
            name
            for name in _cached_placed_object_names(scene_capture_cache)
            if name != selected_name
            and str(name or "").startswith("tingzi_")
            and name not in set(current_rule_target_names)
        ]
        return _dedupe_names(explicit_names + tracked_scene_names + current_rule_target_names + placed_names)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            selected_obstacles = _derive_cycle_obstacle_names(
                base_args,
                cycle_idx,
                selected_name,
                cycle_object_sequence,
